...
- **Ambient tenant exemptions (middleware):** `TenantValidationMiddleware` allows **authenticated SuperAdmin only** to call platform SaaS prefixes without ambient `ICurrentTenantAccessor.TenantId`: `/api/admin/tenants`, `/api/admin/billing`, `/api/admin/cache`, `/api/admin/support`, `/api/admin/trials`, `/api/admin/sessions`, `/api/admin/limits`, `/api/admin/countries` (tenant-selectable country catalog), `/api/admin/rksv/config` (exact overlay API; not `/api/admin/rksv/*` DEP), `/api/admin/backup`, `/api/admin/restore-verification`, `/api/admin/restore` (segment-safe; not `restore-verification`), `/api/settings/backup`, and exact `/api/tenants/switcher`. Segment-safe prefixes (no `/api/admin/tenantsfoo`). **Non–SuperAdmin** on those URLs still need ambient tenant (404). Mandant data APIs (`/api/admin/products`, POS, etc.) always require ambient tenant — even for SuperAdmin. Super Admin backup ACL is deployment-wide even when the JWT has an ambient tenant (`BackupRunAccessScope.IsDeploymentWide`).
...
- **Session management:** Super Admin and Mandanten-Admin (`Manager`) can list/terminate `auth_sessions` including POS (`/api/admin/sessions`, FA `/admin/sessions`). List requires `user.view`; logout requires `user.manage`. Manager is limited to the ambient tenant; Super Admin sees all tenants. The caller’s current session cannot be terminated. Super Admin force-logout (user detail) rotates Identity `SecurityStamp` (JWT `sst` claim) and revokes refresh tokens; the next API call fails in JWT `OnTokenValidated`. Platform/tenant “terminate all” keeps the caller’s current session. User logout (FA/POS cookie split, CSRF, client cleanup): [`docs/AUTH_LOGOUT.md`](docs/AUTH_LOGOUT.md).
...
Austria (`company_settings.country = 'AT'`) remains the production fiscal path (RKSV/TSE/FinanzOnline). Other countries use `CountryProfile` + feature flags; DE/CH/EU modules are **shape-only** and not production-ready. Hub: [`docs/COUNTRIES.md`](docs/COUNTRIES.md). Stubs: [`docs/FISCAL_GERMANY.md`](docs/FISCAL_GERMANY.md), [`docs/FISCAL_SWITZERLAND.md`](docs/FISCAL_SWITZERLAND.md), [`docs/EINVOICING_EU.md`](docs/EINVOICING_EU.md). Cutover: [`docs/COUNTRY_LAYER_CUTOVER.md`](docs/COUNTRY_LAYER_CUTOVER.md). Remaining (NOT STARTED): Paket 20 DE TSE provider, 21 CH QR bank, 22 EU Peppol — [`docs/COUNTRIES.md`](docs/COUNTRIES.md) §16. Paket 30-d (OSS destination rates) is done.

- Resolve country from `CompanySettings.Country` (column `country`, `NOT NULL DEFAULT 'AT'`). Unknown/legacy value = AT. **Never add a `CountryCode` column or a second country field** — `Country`, `VatRegime`, `BillingCountry`, and `TaxExempt` shipped in migration `20260916110000_AddCompanySettingsCountryBilling`. Issue-time snapshots live on invoices/receipts/payment_details (`CountryCodeAtIssue` / `VatRegimeAtIssue`); do not rewrite historical rows on country change.
- `ICountryProfileRegistry` (seeds: AT, DE, CH, registry-only `EU_DEFAULT`) is **shipped and called** (strategy resolvers, `IFeatureFlagService` country defaults, `GET /api/admin/countries`, `IVatIdValidator`). `GetOrDefault` → AT for unknown/legacy codes; `Get` throws `UNKNOWN_COUNTRY_CODE`. Profiles hold no VAT rates. AT seed mirrors the live company-settings defaults and is pinned by tests.
- `ITaxStrategy` / `IInvoiceStrategy` are **wired** into `PaymentService` and `InvoiceService` (Paket 30-c). `TseService` tax-set projection and `RksvSpecialReceiptService` stay Austria-only (`NotSupportedException`). Offline BelegNr goes through `IInvoiceStrategy.AllocateReceiptNumberAsync` (Paket 30-b). AT + `EU_REVERSE_CHARGE` routes to EuDefault strategies (Paket 12-c). Unknown pairs throw `UNKNOWN_TAX_REGIME` (never an AT fallback). Do not fork `PaymentService` or `TseService` internals.
- The Austrian strategies are **adapters only**: `CalculateTax` → `CartMoneyHelper`, `ProjectFiscalTaxSets` → `RksvTaxSetMapper`, `AllocateReceiptNumberAsync` → `ISequenceReservationService`, `BuildInvoiceDocumentAsync` → `IReceiptService`, VAT-ID shape → `CountryProfile`. Never restate a rate, rounding rule, or bucket rule inside a strategy. TSE signing input / machine code / QR stay out of `IInvoiceStrategy`. DE/CH `CalculateTax` uses `ICountryTaxTypeRegistry`; DE/CH/EU `ProjectFiscalTaxSets` and `AllocateReceiptNumberAsync` still throw `NotImplementedException`.
- Gate country features with `IFeatureFlagService` (`tenant_settings` keys `FeatureFlags:{Name}`). Country-derived defaults: tenant override → profile. `Fiscal.RksvAt` is always on for AT and cannot be disabled (**locked**; name is in `FeatureFlagNames`).
- Tenant create requires an ISO 3166-1 alpha-2 country in the request payload, persisted to `company_settings.country` (FA `CreateTenantWizard`: country step then form). Super Admin tenant detail: Country & Fiscal Regime card (`PATCH /api/admin/tenants/{id}/country`). Non-AT tenants must not enable RKSV/TSE.
- VAT-ID regexes, rates, and labels live in CountryProfile seeds / `ICountryTaxTypeRegistry` — not in controllers or FA form constants. `EU_DEFAULT` is registry-only, not selectable.
- **One VAT-ID literal:** `Models/Countries/VatIdPatterns` (`Austria` = `^ATU\d{8}$`, …) is referenced by the CountryProfile seeds, the two `[RegularExpression]` attributes, and every runtime consumer (`PaymentService`, `TenantSettingsService`, fiskaly services, `InvoiceController`). Never declare a local `^ATU\d{8}$` again — a test scans for it. Matching is **strict** (no trim, no case folding); normalization stays at the call site that needs it.
...
- Optional storage tiers (`Backup:StorageTierManagementEnabled`): Hot ≤30d / Warm ≤90d / Cold &gt;90d via `StorageTierService` (Super Admin FA `/backup/retention`). Cold upload is explicit via `ICloudStorageService` (Filesystem WORM, S3 Glacier, Azure Archive, GCS Coldline). System backups keep a 7-year Legal Hold (BAO §132).
...
- **Country / VAT / e-invoicing work** → `docs/COUNTRIES.md` + `docs/FISCAL_GERMANY.md` + `docs/FISCAL_SWITZERLAND.md` + `docs/EINVOICING_EU.md` + `docs/COUNTRY_LAYER_CUTOVER.md`. Remaining gaps (NOT STARTED): Paket 20/21/22 in `docs/COUNTRIES.md` §16. Paket 30-d (OSS destination rates) is done.
