...
- `ITaxStrategy` / `IInvoiceStrategy` (`backend/Services/Countries/Strategies/`) are **shipped but not yet called in production paths** — `PaymentService`, `TseService`, `ReceiptService`, and `RksvSpecialReceiptService` are untouched. Select behavior from `CountryProfile` + `VatRegime` via `ITaxStrategyResolver` / `IInvoiceStrategyResolver`; unknown pairs throw `UNKNOWN_TAX_REGIME` (never an AT fallback). Do not fork `PaymentService` or `TseService` internals.
- The Austrian strategies are **adapters only**: `CalculateTax` → `CartMoneyHelper`, `ProjectFiscalTaxSets` → `RksvTaxSetMapper`, `AllocateReceiptNumberAsync` → `ISequenceReservationService`, `BuildInvoiceDocumentAsync` → `IReceiptService`, VAT-ID shape → `CountryProfile`. Never restate a rate, rounding rule, or bucket rule inside a strategy. TSE signing input / machine code / QR stay out of `IInvoiceStrategy`. DE/CH/EU strategies throw `NotImplementedException`.
