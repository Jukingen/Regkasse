...
internal static class ApplicationHost
{
    ...
    private static bool IsTrustedDevelopmentCorsOrigin(string? origin)
    {
        ...
        if (uri.Scheme != Uri.UriSchemeHttp && uri.Scheme != Uri.UriSchemeHttps)
            return false;
        if (TenantHostNames.IsTrustedLocalDevCorsHost(uri.Host))
            return true;
    ...
    /// <summary>
    /// Production CORS: exact <c>Cors:AllowedOrigins</c> entries, or HTTPS origins on <c>*.regkasse.at</c>
    /// (POS / FA / shared Sites / tenant slug hosts). Custom domains must be listed explicitly.
    /// </summary>
    private static bool IsTrustedProductionCorsOrigin(string? origin, IReadOnlySet<string> configuredOrigins)
    {
        if (string.IsNullOrWhiteSpace(origin))
            return false;
        ...
        if (uri.Scheme != Uri.UriSchemeHttps)
            return false;
        return TenantHostNames.IsRegkassePlatformWebOriginHost(uri.Host);
    }
...
#pragma warning restore CS0618
        ...
        builder.Services.AddHostedService<TenantLifecycleArchiveHostedService>();
        ...
        builder.Services.AddScoped<ITseDeveloperToolsService, TseDeveloperToolsService>();
        ...
        builder.Services.AddScoped<ITseCostOptimizationService, TseCostOptimizationService>();
        builder.Services.AddScoped<ITsePredictiveAnalyticsService, TsePredictiveAnalyticsService>();
        builder.Services.AddScoped<ITseResourcePoolService, TseResourcePoolService>();
        builder.Services.AddScoped<ITseIncidentService, TseIncidentService>();
        builder.Services.AddScoped<ITseSlaMonitorService, TseSlaMonitorService>();
        builder.Services.AddScoped<ITseCapacityPlanningService, TseCapacityPlanningService>();
        builder.Services.AddScoped<ITseAutoScalingService, TseAutoScalingService>();
        builder.Services.AddScoped<ITseAutoHealingService, TseAutoHealingService>();
        builder.Services.AddScoped<ITseKnowledgeBaseService, TseKnowledgeBaseService>();
        builder.Services.AddSingleton<ITseGatewayMetricsStore, TseGatewayMetricsStore>();
        builder.Services.AddScoped<ITseApiGatewayService, TseApiGatewayService>();
        builder.Services.AddScoped<ITseAnomalyDetectionService, TseAnomalyDetectionService>();
        builder.Services.AddScoped<ITseReportingService, TseReportingService>();
        builder.Services.AddScoped<ITseUserAnalyticsService, TseUserAnalyticsService>();
        builder.Services.AddScoped<ITseRecommendationService, TseRecommendationService>();
        builder.Services.AddScoped<ITseUpdateService, TseUpdateService>();
        builder.Services.AddScoped<ITseSustainabilityService, TseSustainabilityService>();
        builder.Services.AddScoped<ITseWebhookService, TseWebhookService>();
        builder.Services.AddScoped<ITseBlockchainService, TseBlockchainService>();
        builder.Services.AddSingleton<ITseTrainingConsoleStore, TseTrainingConsoleStore>();
        builder.Services.AddScoped<ITseTrainingService, TseTrainingService>();
        builder.Services.AddHttpClient(TseWebhookService.HttpClientName, client =>
        {
            client.Timeout = TimeSpan.FromSeconds(30);
        });
        builder.Services.AddScoped<ITenantTsePortalService, TenantTsePortalService>();
        builder.Services.AddScoped<ITseDisasterRecoveryService, TseDisasterRecoveryService>();
        builder.Services.AddScoped<ITseLogAggregationService, TseLogAggregationService>();
        builder.Services.AddScoped<ITseMetricsService, TseMetricsService>();
        builder.Services.AddScoped<ITseHealthCheckService, TseFleetHealthCheckService>();
        ...
        // CORS: browser clients only (POS Expo web / FA Next / Sites). Policy name is historical-safe for logs.
        builder.Services.AddCors(options =>
        {
            options.AddPolicy(CorsPolicyName, policy =>
            {
                ...
                {
                    throw new InvalidOperationException(
                        "Cors:AllowedOrigins must be configured in non-development environments (POS/FA/Sites). See backend/CONFIGURATION.md.");
                }

                var allowed = new HashSet<string>(configuredOrigins, StringComparer.Ordinal);
        ...
        {
            // Single document "v1": all discovered controllers are included unless excluded below
            // (legacy aliases via DocInclusionPredicate, or [ApiExplorerSettings(IgnoreApi = true)]).
            ...
            c.CustomSchemaIds(OpenApiSchemaIdSelector.Select);
        ...
        // Null/Start/Schluss/Tagesabschluss *ReportService* types remain for composers/tests; text path is IRksvReportTextService.
    ...
    public static async Task RunAsync(WebApplication app)
    {
        ...
        Console.WriteLine($"=== SWAGGER: http://localhost:{devPortForBanner}/swagger ===");
