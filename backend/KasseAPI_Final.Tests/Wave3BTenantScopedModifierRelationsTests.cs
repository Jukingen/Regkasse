...
public sealed class Wave3BTenantScopedModifierRelationsTests
{
    ...
    public async Task SameModifierSetup_AllowedPerTenant_IsolatedRows()
    {
        ...
        var forA = await ctx.ProductModifierGroupAssignments.CountAsync(a => a.TenantId == TenantA);
        var forB = await ctx.ProductModifierGroupAssignments.CountAsync(a => a.TenantId == TenantB);
        Assert.Equal(1, forA);
        Assert.Equal(1, forB);
    }
