...
/// <summary>
/// Zamanlanmış restore drill sıraya alma: başarılı kanıt (CompletedAt) ve Scheduled Queued/Running dedup.
/// </summary>
public sealed class RestoreVerificationWeeklySchedulingTests
{
    ...
    private static RestoreVerificationOptions SchedulingEnabledOptions(int cadenceDays = 7, bool manualCounts = false) =>
        new()
        {
            ScheduledWeeklyDrillEnabled = true,
            ScheduledProofCadenceDays = cadenceDays,
            ManualSuccessSatisfiesScheduledProofCadence = manualCounts
        };
    ...
    [Fact]
    public async Task Scheduler_enqueues_when_only_manual_success_exists_and_policy_counts_manual_false()
    {
        ...
        var orchestrator = CreateSchedulingOrchestrator(sf, SchedulingEnabledOptions(manualCounts: false));
        await orchestrator.TryEnqueueWeeklyIfDueExclusiveBodyAsync(CancellationToken.None);
    ...
    [Fact]
    public async Task Scheduler_skips_when_manual_success_within_window_and_policy_counts_manual_true()
    {
        var dbName = $"rv_manual_counts_{Guid.NewGuid():N}";
        var sf = CreateScopeFactory(dbName);
        await using (var scope = sf.CreateAsyncScope())
        {
            var db = scope.ServiceProvider.GetRequiredService<AppDbContext>();
        ...
        var orchestrator = CreateSchedulingOrchestrator(sf, SchedulingEnabledOptions(manualCounts: true));
        await orchestrator.TryEnqueueWeeklyIfDueExclusiveBodyAsync(CancellationToken.None);
