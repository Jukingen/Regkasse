...
public sealed class ManualRestoreTargetDatabaseGuardTests
{
    ...
    {
        // Non-prefixed names (including DefaultConnection database) fail the validation prefix gate first.
        var guard = CreateGuard(database: "regkasse_prod");
    ...
    [Fact]
    public void ValidateOrThrow_rejects_prefixed_name_listed_as_blocked()
    {
        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["ConnectionStrings:DefaultConnection"] =
                    "Host=localhost;Database=regkasse_prod;Username=u;Password=p",
            })
            .Build();
        ...
        var options = Options.Create(new ManualRestoreApprovalOptions
        {
            ...
            AdditionalBlockedDatabaseNames = new[] { "restore_validation_blocked_clone" },
        });

        var guard = new ManualRestoreTargetDatabaseGuard(config, new OptionsMonitorStub(options.Value));
        var ex = Assert.Throws<ArgumentException>(() =>
            guard.ValidateOrThrow("restore_validation_blocked_clone"));
