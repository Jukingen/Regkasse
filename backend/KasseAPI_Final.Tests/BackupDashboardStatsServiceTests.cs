...
public sealed class BackupDashboardStatsServiceTests
{
        ...
        DateTime utcNow)
    {
        ...
        services.AddSingleton<IBackupStagingDiskMonitor, BackupStagingDiskMonitor>();
        services.AddSingleton<Microsoft.Extensions.Options.IOptionsMonitor<BackupOptions>>(_ =>
        {
            var opts = new BackupOptions { ArtifactStagingRoot = null };
            return new FixedOptionsMonitor<BackupOptions>(opts);
        });
