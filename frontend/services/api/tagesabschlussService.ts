...
function readErrorPayload(
  error: unknown
): { error?: string; paymentsWithoutInvoiceCount?: number } | null {
...
export const performDailyClosing = async (
  request: DailyClosingRequest
): Promise<TagesabschlussResult> => {
  try {
 ...
 */
export const performMonthlyClosing = async (
  request: DailyClosingRequest
): Promise<TagesabschlussResult> => {
  try {
 ...
 */
export const performYearlyClosing = async (
  request: DailyClosingRequest
): Promise<TagesabschlussResult> => {
  try {
...
export async function downloadClosingReportPdf(closingId: string, language = 'de'): Promise<Blob> {
...
export const formatClosingTime = (dateString: string): string =>
  formatUserDateTime(dateString, { includeSeconds: true });
