...
export const useOrders = () => {
  ...
  const createOrder = useCallback(
    async (tableNumber: number, customerId?: string) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const addItemToOrder = useCallback(
    async (tableNumber: number, productId: string, quantity: number, notes?: string) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const updateOrderItemQuantity = useCallback(
    async (tableNumber: number, itemId: string, quantity: number) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const cancelOrder = useCallback(
    async (tableNumber: number, reason?: string) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const updateOrderNotes = useCallback(
    async (tableNumber: number, notes: string) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const updateOrderCustomer = useCallback(
    async (tableNumber: number, customerId: string) => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Müşteri bilgileri güncellenemedi';
        setError(errorMessage);
        throw err;
      } finally {
        setLoading(false);
