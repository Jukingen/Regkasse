...
interface CartContextType {
  ...
  addModifier: (
    cartItemId: string,
    modifier: Omit<CartItemModifier, 'quantity'> & { quantity?: number }
  ) => Promise<void>;
...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Load cart UI state from non-sensitive storage on mount
  useEffect(() => {
    const loadState = async () => {
      try {
        const parsed = await storage.getJson<{
          state?: { activeTableId?: number; cartsByTable?: CartsByTable };
        }>('cart-storage');
        if (parsed?.state) {
  ...
  // Persist cart UI state (non-sensitive operational data)
  useEffect(() => {
    const saveState = async () => {
      try {
        await storage.setJson('cart-storage', {
          state: {
            activeTableId,
            cartsByTable,
          },
  ...
  const getCartForTable = useCallback(
    (tableNumber: number): Cart => {
  ...
  const fetchTableCart = useCallback(
      ...
      try {
        ...
        if (response.cart) {
          ...
          const localItems: CartItem[] = backendItems.map((item: any, index: number) => {
            const backendItemId = item.Id ?? item.id;
            const mods =
              item.SelectedModifiers ?? item.selectedModifiers ?? item.Modifiers ?? item.modifiers;
            let modifierList: CartItemModifier[] | undefined;
            if (Array.isArray(mods) && mods.length) {
              ...
              const existing = existingItems.find(
                (e: CartItem) =>
                  e.itemId === backendItemId ||
                  (e.productId === (item.ProductId || item.productId) && !e.itemId)
              );
            ...
            return {
              ...
              qty,
              unitPrice: baseUnit,
              totalPrice: modifierList?.length
                ? baseUnit * qty + modTotal
                : (item.TotalPrice ?? item.totalPrice ?? baseUnit * qty),
              notes: item.Notes ?? item.notes,
    ...
    async (productId: string, quantity: number) => {
      ...
      try {
        ...
        const updatedItems = currentCart.items.map((i) =>
          i.productId === productId ? { ...i, qty: quantity } : i
        );
        setCartsByTable((prev) => ({
          ...prev,
          [activeTableId]: {
  ...
  const removeByItemId = useCallback(
    async (itemId: string) => {
      ...
      const lineKey = (i: CartItem) =>
        i.itemId ?? i.clientId ?? `${i.productId}-${getModifierKey(i.modifiers)}`;
  ...
  const updateItemQuantityByItemId = useCallback(
  ...
  const incrementModifier = useCallback(
    async (cartItemId: string, modifierId: string) => {
      ...
      const nextMods = mods.map((m) =>
        m.id === modifierId ? { ...m, quantity: (m.quantity ?? 1) + 1 } : m
      );
      const totalPrice = recalcLineTotal({ ...item, modifiers: nextMods });
      const updatedItems = currentCart.items.map((it, i) =>
        i === index ? { ...it, modifiers: nextMods, totalPrice } : it
  ...
  const decrementModifier = useCallback(
    async (cartItemId: string, modifierId: string) => {
      ...
      const nextMods =
        nextQty <= 0
          ? mods.filter((m) => m.id !== modifierId)
          : mods.map((m) => (m.id === modifierId ? { ...m, quantity: nextQty } : m));
      const totalPrice = recalcLineTotal({ ...item, modifiers: nextMods });
      const updatedItems = currentCart.items.map((it, i) =>
  ...
  const splitCartItems = useCallback(
    async (sourceTable: number, targetTable: number, itemIds: string[]) => {
      setLoading(true);
      try {
  ...
  const mergeTableCarts = useCallback(
    async (sourceTable: number, targetTable: number) => {
      setLoading(true);
      try {
