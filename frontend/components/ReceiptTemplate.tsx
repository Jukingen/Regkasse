...
function ReceiptSignatureQr({ payload, size }: { payload?: string; size: number }) {
  const value = payload?.trim() || '';
  const ecl = useMemo(() => (value ? resolveRksvQrEcl(value) : null), [value]);

  if (!value || !ecl) {
    return (
      <View style={[styles.qrContainer, { height: size, width: size }]}>
        <Text style={styles.qrPlaceholderText}>QR Code nicht verfügbar</Text>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.qrContainer, { height: size, width: size }]}>
      <RksvQrCodeSvg value={value} ecl={ecl} size={size - 8} quietZone={4} />
    </View>
  );
}
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <View style={styles.itemsContainer}>
        ...
        {items.map((item, index) => (
          <View
            ...
            style={[styles.itemRow, item.isModifierLine && styles.modifierRow]}>
            <Text
              style={[styles.itemText, styles.flex2, item.isModifierLine && styles.modifierText]}>
              {item.isModifierLine && !item.name?.startsWith('+')
                ? `+ ${item.name}`
                : (item.name ?? '')}
            </Text>
            <Text
              style={[
                styles.itemText,
                styles.flexRight,
                item.isModifierLine && styles.modifierText,
              ]}>
              {item.quantity}
            </Text>
            <Text
            ...
            <Text
              ...
              {item.taxRate >= 20 ? 'A' : item.taxRate >= 10 ? 'B' : 'C'}
            </Text>
          </View>
        ))}
      </View>
        ...
        {taxRates.map((rate, index) => (
          <View key={index} style={styles.vatRow}>
            <Text style={styles.vatText}>
              {rate.rate}% ({rate.rate >= 20 ? 'A' : rate.rate >= 10 ? 'B' : 'C'})
            </Text>
        ...
        {payments.map((payment, index) => (
          <View key={index} style={styles.paymentRow}>
            <Text style={styles.paymentMethod}>
              {payment.method === 'cash' ? 'Barzahlung' : payment.method}
            </Text>
          ...
          <>
            <View style={styles.paymentRow}>
              ...
              <Text style={styles.paymentAmount}>
                {formatCurrency(payments.find((p) => p.method === 'cash')?.tendered || 0)} EUR
              </Text>
            </View>
            <View style={styles.paymentRow}>
              <Text style={styles.paymentMethod}>Rückgeld</Text>
              <Text style={styles.paymentAmount}>
                {formatCurrency(payments.find((p) => p.method === 'cash')?.change || 0)} EUR
              </Text>
            </View>
          </>
        )}
      ...
      <View style={styles.signatureBlock}>
        ...
        {signature && (
          <>
            <ReceiptSignatureQr payload={signature.qrData} size={150} />
            <Text style={styles.signatureText}>{formatTseSignatureDisplay(signature?.value)}</Text>
            <Text style={styles.signatureMeta}>
              {signature.serialNumber} | {signature.timestamp}
            </Text>
          </>
        )}
      </View>
