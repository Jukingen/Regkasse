    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.receipt}>
        ...
        <View style={styles.header}>
          <Text style={styles.companyName}>DEMO GMBH</Text>
          <Text style={styles.address}>Hauptstraße 1, 1010 Wien</Text>
          <Text style={styles.taxNumber}>UID: ATU12345678</Text>
        </View>
        ...
        <View style={styles.receiptInfo}>
          ...
          <Text style={styles.receiptDate}>
            Datum: {formatDate(new Date(invoice.createdAt))}
          </Text>
          <Text style={styles.receiptTime}>
            Uhrzeit: {formatTime(new Date(invoice.createdAt))}
          </Text>
        </View>
        ...
        <View style={styles.taxSection}>
          ...
          {standardTaxAmount > 0 && (
            <View style={styles.taxRow}>
              <Text style={styles.taxLabel}>MwSt. 20%:</Text>
              <Text style={styles.taxAmount}>€{standardTaxAmount.toFixed(2)}</Text>
            </View>
          )}
          {reducedTaxAmount > 0 && (
            <View style={styles.taxRow}>
              <Text style={styles.taxLabel}>MwSt. 10%:</Text>
              <Text style={styles.taxAmount}>€{reducedTaxAmount.toFixed(2)}</Text>
            </View>
          )}
          {specialTaxAmount > 0 && (
            <View style={styles.taxRow}>
              <Text style={styles.taxLabel}>MwSt. 13%:</Text>
              <Text style={styles.taxAmount}>€{specialTaxAmount.toFixed(2)}</Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.tseSection}>
          ...
          <Text style={styles.tseInfo}>
            TSE-Zeitstempel: {formatDate(new Date(invoice.tseTime))} {formatTime(new Date(invoice.tseTime))}
          </Text>
