...
const OrderManager: React.FC<OrderManagerProps> = ({
  ...
  const createNewOrder = () => {
    if (newOrderItems.length === 0) {
      Alert.alert(t('order.error'), t('order.no_items'));
      return;
    }
  ...
  const completeOrder = (order: Order) => {
    Alert.alert(
      t('order.complete_title'),
      t('order.complete_message'),
      [
        { text: t('common.cancel'), style: 'cancel' },
        {
          text: t('order.complete'),
          onPress: () => {
  ...
  const renderOrderItem = ({ item: order }: { item: Order }) => (
    ...
    >
      ...
      <View style={styles.orderInfo}>
        ...
        {order.tableNumber && (
          <Text style={styles.tableNumber}>{t('order.table')} {order.tableNumber}</Text>
        )}
      </View>
      ...
      <View style={styles.orderItems}>
        ...
        {order.items.length > 2 && (
          <Text style={styles.moreItemsText}>
            +{order.items.length - 2} {t('order.more_items')}
          </Text>
        )}
      </View>
    ...
    >
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>{t('order.manager')}</Text>
          <View style={styles.headerActions}>
            ...
            >
              ...
              <Text style={styles.newOrderButtonText}>{t('order.new')}</Text>
            </TouchableOpacity>
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateText}>
                {currentUserRole === 'admin' ? t('order.no_orders') : t('order.no_my_orders')}
              </Text>
            </View>
          }
        />
        ...
        >
          <View style={styles.newOrderContainer}>
            <View style={styles.newOrderHeader}>
              <Text style={styles.newOrderTitle}>{t('order.create_new')}</Text>
            ...
            <ScrollView style={styles.newOrderContent}>
              ...
              <View style={styles.customerSection}>
                <Text style={styles.sectionTitle}>{t('order.customer_info')}</Text>
                <TextInput
                  style={styles.input}
                ...
                <TextInput
                  style={styles.input}
                  placeholder={t('order.table_number_placeholder')}
                  value={tableNumber}
              ...
              <View style={styles.productsSection}>
                <Text style={styles.sectionTitle}>{t('order.select_products')}</Text>
                  ...
                  {newOrderItems.map((item, index) => (
                    <View key={`${item.product.id}-${index}`} style={styles.selectedItem}>
                      ...
                      <TextInput
                        ...
                        placeholder={t('order.item_notes_placeholder')}
                        value={item.notes}
              ...
              <View style={styles.notesSection}>
                <Text style={styles.sectionTitle}>{t('order.order_notes')}</Text>
                <TextInput
                  style={[styles.input, styles.notesTextArea]}
                  placeholder={t('order.order_notes_placeholder')}
                  value={orderNotes}
            ...
            <View style={styles.newOrderFooter}>
              ...
              >
                ...
                <Text style={styles.createOrderButtonText}>{t('order.create')}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </Modal>
