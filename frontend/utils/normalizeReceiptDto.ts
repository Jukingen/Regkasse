...
function normalizeSignature(raw: Record<string, unknown> | undefined | null): ReceiptDTO['signature'] {
...
export function normalizeReceiptDto(raw: unknown): ReceiptDTO {
  ...
  return {
    ...
    kassenID: String(r.kassenID ?? r.KassenID ?? r.displayRegisterNumber ?? r.DisplayRegisterNumber ?? ''),
    items,
    taxRates,
    ...
    originalSaleReceiptId: (r.originalSaleReceiptId ?? r.OriginalSaleReceiptId ?? null) as string | null,
  };
}
