...
1. After backend OpenAPI changes: `node scripts/generate-backend-openapi.mjs` (repo root), then `npm run generate:api` (orval); commit `backend/swagger.json` + regenerated `src/api/generated/**`. Confirm with `npm run verify:api-client` (or root `node scripts/verify-api-client.mjs`). CI: `api-client-alignment.yml`.
2. Run `npm run build` (includes `tsc` via Next) and `npm test`.
3. Run `npm run lint` — includes a fast OpenAPI/orval-config check; contract surfaces use stricter `@typescript-eslint/no-explicit-any` (warn); prefer fixing new warnings before release.
