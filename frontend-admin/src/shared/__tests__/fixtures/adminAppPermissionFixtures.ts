/**
 * Admin JWT permission fixtures — must stay aligned with
 * `backend/Authorization/AdminAppPermissionProfile.cs` + `RolePermissionMatrix.cs`.
 * Manager oversight reads: `AdminAppPermissionProfile.ManagerOversightViewPermissions`.
 * Manager menu contract: `MANAGER_REQUIRED_MENU_KEYS` / `MANAGER_FORBIDDEN_MENU_KEYS`.
 * Used by role ↔ menu visibility contract tests (CI guardrail).
 */
import { PERMISSIONS } from '@/shared/auth/permissions';

/** Cashier after AdminAppPermissionProfile.Filter(app=admin). */
export const CASHIER_ADMIN_PERMISSIONS: readonly string[] = [
  PERMISSIONS.PRODUCT_VIEW,
  PERMISSIONS.CATEGORY_VIEW,
  PERMISSIONS.MODIFIER_VIEW,
  PERMISSIONS.PAYMENT_VIEW,
  PERMISSIONS.REPORT_VIEW,
];

/**
 * Representative Manager admin permissions (matrix minus POS-terminal write strip).
 * Keep in sync with backend `RolePermissionMatrix` + `AdminAppPermissionProfile.Filter`.
 */
export const MANAGER_ADMIN_PERMISSIONS: readonly string[] = [
  PERMISSIONS.USER_VIEW,
  PERMISSIONS.USER_MANAGE,
  PERMISSIONS.USER_RESET_PASSWORD,
  PERMISSIONS.ROLE_VIEW,
  PERMISSIONS.PRODUCT_VIEW,
  PERMISSIONS.PRODUCT_MANAGE,
  PERMISSIONS.CATEGORY_VIEW,
  PERMISSIONS.MODIFIER_VIEW,
  PERMISSIONS.PAYMENT_VIEW,
  PERMISSIONS.PAYMENT_CANCEL,
  PERMISSIONS.REFUND_CREATE,
  PERMISSIONS.SALE_VIEW,
  PERMISSIONS.REPORT_VIEW,
  PERMISSIONS.REPORT_EXPORT,
  PERMISSIONS.DAILY_CLOSING_VIEW,
  PERMISSIONS.DAILY_CLOSING_EXECUTE,
  PERMISSIONS.CASHREGISTER_VIEW,
  PERMISSIONS.CASHREGISTER_MANAGE,
  PERMISSIONS.SETTINGS_VIEW,
  PERMISSIONS.BACKUP_MANAGE,
  PERMISSIONS.WEBSITE_MANAGE,
  PERMISSIONS.DIGITAL_VIEW,
  PERMISSIONS.DIGITAL_PREVIEW,
  PERMISSIONS.DIGITAL_REQUEST,
  PERMISSIONS.DIGITAL_ORDERS_VIEW,
  PERMISSIONS.DIGITAL_ORDERS_MANAGE,
  PERMISSIONS.DIGITAL_WEB_VIEW,
  PERMISSIONS.DIGITAL_WEB_USE,
  PERMISSIONS.DIGITAL_APP_VIEW,
  PERMISSIONS.DIGITAL_APP_USE,
  PERMISSIONS.LICENSE_MANAGE,
  PERMISSIONS.AUDIT_VIEW,
  PERMISSIONS.FINANZONLINE_MANAGE,
  PERMISSIONS.FINANZONLINE_VIEW,
  PERMISSIONS.TABLE_VIEW,
  PERMISSIONS.ORDER_VIEW,
  PERMISSIONS.SHIFT_VIEW,
  PERMISSIONS.INVENTORY_VIEW,
  PERMISSIONS.INVENTORY_MANAGE,
  PERMISSIONS.INVOICE_VIEW,
  PERMISSIONS.CUSTOMER_VIEW,
  PERMISSIONS.CART_VIEW,
  PERMISSIONS.VOUCHER_READ,
  PERMISSIONS.FISCAL_EXPORT_COMPLIANCE,
  PERMISSIONS.RKSV_NULLBELEG_CREATE,
  PERMISSIONS.RKSV_STARTBELEG_CREATE,
  PERMISSIONS.RKSV_MONATSBELEG_CREATE,
  PERMISSIONS.RKSV_JAHRESBELEG_CREATE,
  PERMISSIONS.RKSV_SCHLUSSBELEG_CREATE,
  PERMISSIONS.FISKALY_OPERATIONS_VIEW,
  PERMISSIONS.FISKALY_OPERATIONS_NORMAL,
  PERMISSIONS.FISKALY_OPERATIONS_CANCEL,
  PERMISSIONS.FISKALY_OPERATIONS_NULLBELEG,
  PERMISSIONS.FISKALY_OPERATIONS_STARTBELEG,
  PERMISSIONS.FISKALY_OPERATIONS_MONATSBELEG,
  PERMISSIONS.FISKALY_OPERATIONS_JAHRESBELEG,
  PERMISSIONS.FISKALY_OPERATIONS_SCHLUSSBELEG,
  PERMISSIONS.FISKALY_OPERATIONS_TAGESABSCHLUSS,
  PERMISSIONS.FISKALY_OPERATIONS_DEP_EXPORT,
  PERMISSIONS.FISKALY_HISTORY_VIEW,
  PERMISSIONS.FISKALY_HISTORY_RETRY,
];

/** Menus Cashier must never see in FA (POS-only or admin-only). */
export const CASHIER_FORBIDDEN_MENU_KEYS: readonly string[] = [
  '/tables',
  '/receipts',
  '/shifts',
  '/tagesabschluss',
  '/operations-center',
  '/kassenverwaltung',
  '/users',
  '/admin/users',
  '/admin/sessions',
  '/settings',
  '/admin/license',
  '/admin/license-management',
  '/admin/limits/dashboard',
  '/tenant/dashboard',
  '/tenant/portal',
  '/tenant/invoices',
  '/tenant/support',
  '/tenant/license',
  '/tenant/profile',
  '/rksv/operations',
  '/audit-logs',
];

/** Menus Cashier must see with admin-filtered permissions. */
export const CASHIER_REQUIRED_MENU_KEYS: readonly string[] = [
  '/dashboard',
  '/products',
  '/payments',
  '/reporting/report-center',
];

/**
 * Manager oversight menus — must remain visible with `MANAGER_ADMIN_PERMISSIONS`.
 * Grouped by sidebar IA; update when adding catalog/RKSV leaves Manager should reach.
 */
export const MANAGER_REQUIRED_MENU_KEYS: readonly string[] = [
  '/dashboard',
  '/operations-center',
  '/tables',
  '/kassenverwaltung',
  '/shifts',
  '/tagesabschluss',
  '/staff',
  '/receipts',
  '/orders/online',
  '/payments',
  '/payments/storno-refund-audit',
  '/payments/trends',
  '/admin/payments/card-transactions',
  '/vouchers',
  '/invoices',
  '/products',
  '/categories',
  '/modifier-groups',
  '/pricing-rules',
  '/inventory',
  '/customers',
  '/reporting',
  '/reporting/report-center',
  '/reporting/staff',
  '/admin/reports',
  '/audit-logs/activity',
  '/admin/reports/user-activity',
  '/audit-logs',
  '/audit-logs/activity',
  '/admin/audit/fiscal-exports',
  '/admin/tse/offline-transactions',
  '/admin/tse/signaturkarte-program',
  '/rksv/sb/startbeleg',
  '/rksv/sb/monatsbeleg',
  '/rksv/sb/jahresbeleg',
  '/rksv/sb/nullbeleg',
  '/rksv/sb/schlussbeleg',
  '/rksv/operations',
  '/rksv/status',
  '/rksv/finanz-online-outbox',
  '/rksv/offline-orders',
  '/rksv/finanz-online-queue',
  '/rksv/finanz-online-operations',
  '/rksv/incident',
  '/rksv/replay-batch',
  '/rksv/payload-hash-conflicts',
  '/rksv/verifications',
  '/rksv/fiscal-export-diagnostics',
  '/rksv/integrity',
  '/rksv/compliance',
  '/rksv/dep-export-compliance',
  '/rksv/signature-chain',
  '/rksv/offline-intent-coverage',
  '/rksv/belegcheck',
  '/rksv/cmc-certificate',
  '/admin/rksv/dep-export',
  '/admin/rksv/signature-verify',
  '/admin/fiskaly/history',
  '/admin/fiskaly/statistics',
  '/admin/fiskaly/errors',
  '/admin/users',
  '/admin/sessions',
  '/admin/access',
  '/admin/access/matrix',
  '/settings',
  '/settings/receipt',
  '/settings/session',
  '/settings/sessions',
  '/settings/personalization',
  '/settings/payment-methods',
  '/settings/website',
  '/settings/digital',
  '/settings/data-management',
  '/settings/account',
  '/digital/customer-portal',
  '/backup',
  '/backup/runs',
  '/admin/license-management',
  '/admin/limits/dashboard',
  '/tenant/portal',
  '/tenant/invoices',
  '/tenant/support',
  '/tenant/license',
  '/tenant/profile',
];

/**
 * Manager must not see: POS floor signing, platform admin, benefits, receipt templates, dev tooling.
 */
export const MANAGER_FORBIDDEN_MENU_KEYS: readonly string[] = [
  '/admin/access/roles',
  '/receipt-templates',
  '/receipt-generate',
  '/benefit-definitions',
  '/benefit-assignments',
  '/settings/development-mode',
  '/settings/company',
  '/settings/offline',
  '/settings/payment',
  '/settings/tse',
  '/settings/finanzonline',
  '/admin/system/time-sync',
  '/admin',
  '/admin/tenants',
  '/admin/tenants/create',
  '/admin/data-management',
  '/admin/licenses',
  '/admin/cash-registers',
  '/admin/billing',
  '/billing/digital',
  '/admin/digital',
  '/admin/digital/requests',
  '/admin/online-payments',
  '/admin/payments/gateway-console',
  '/admin/feedback',
  '/admin/support',
  '/admin/tse-management',
  '/admin/rksv/config',
  '/admin/tse/failover',
  '/admin/tse/resource-pools',
  '/admin/tse/incidents',
  '/admin/tse/sla',
  '/admin/tse/capacity',
  '/admin/tse/cost',
  '/admin/tse/disaster-recovery',
  '/admin/tse/logs',
  '/admin/tse/developer-tools',
  '/admin/tse/fiskaly',
  '/admin/tse/fiskaly/setup',
  '/admin/tse/fiskaly/test',
  '/admin/tse/compliance',
  '/admin/tse/auto-scaling',
  '/admin/tse/auto-healing',
  '/admin/tse/knowledge',
  '/admin/tse/api-gateway',
  '/admin/tse/anomalies',
  '/admin/tse/analytics',
  '/admin/tse/user-analytics',
  '/admin/tse/recommendations',
  '/admin/tse/updates',
  '/admin/tse/sustainability',
  '/admin/tse/webhooks',
  '/admin/tse/blockchain',
  '/admin/tse/training',
  '/admin/approvals',
  '/admin/maintenance',
  '/admin/feature-flags',
  '/admin/deployments',
  '/admin/deployments/go-live',
  '/admin/errors',
  '/admin/monitoring',
  '/admin/risk-dashboard',
  '/rksv/sb/test-helper',
];
