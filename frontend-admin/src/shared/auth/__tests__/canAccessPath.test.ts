...
describe('canAccessPath', () => {
  ...
  it('denies Cashier without finanzonline.manage', () => {
    expect(
      canAccessPath('/rksv/sonderbelege', [PERMISSIONS.PAYMENT_VIEW, PERMISSIONS.REPORT_VIEW])
    ).toBe(false);
  });
  ...
  it('allows all MANAGER_REQUIRED_MENU_KEYS with manager admin permissions', () => {
    ...
    expect(denied, `Route guard blocks manager oversight paths: ${denied.join(', ')}`).toEqual([]);
  });
  ...
  it('denies platform admin paths even when cash_register.view is present', () => {
    expect(
      canAccessPath('/admin/tenants', [PERMISSIONS.CASHREGISTER_VIEW, PERMISSIONS.USER_VIEW])
    ).toBe(false);
  });
