 ...
 */
export function viewAuditLogStatusPresentation(
  status: AuditLogEntryDto['status'] | number
): AuditLogStatusPresentation {
  ...
  if (typeof status === 'number' && Number.isInteger(status)) {
    ...
    if (idx === 1 || idx === 6 || idx === 7 || idx === 8 || idx === 10)
      return { label, antColor: 'error' };
