...
function resolveOperationalRegisterId(
    ...
    const preferred = pickPreferredCashRegisterId(registers, currentId, tenantId);
    if (preferred) {
        return preferred;
    }

    const scoped = tenantId ? registers.filter((row) => row.tenantId === tenantId) : registers;
    return scoped[0]?.id ?? registers[0]?.id ?? null;
}
...
function resolveInitialSelection(
    ...
    if (autoSelect) {
        return resolveOperationalRegisterId(
            registers,
...
export function useCashRegisterSelection(options: UseCashRegisterSelectionOptions = {}) {
    ...
    const selectedRegisterId = isControlled ? value : internalId;
