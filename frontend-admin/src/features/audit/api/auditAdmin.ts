...
): Promise<{ kind: 'immediate'; blob: Blob; fileName: string } | { kind: 'background'; jobId: string; matchedRows?: number }> {
...
export async function pollAuditExportJob(jobId: string): Promise<AuditExportJobStatus> {
    const res = await AXIOS_INSTANCE.get<AuditExportJobStatus>(`/api/admin/audit/export/jobs/${jobId}`);
    return res.data;
}
