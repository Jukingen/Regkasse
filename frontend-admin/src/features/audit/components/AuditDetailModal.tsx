  ...
  return (
    ...
    >
      <Descriptions size="small" column={2} bordered>
        ...
        <Descriptions.Item label={t('common.auditLogs.table.action')}>
          {record.action ?? '—'}
        </Descriptions.Item>
      ...
      {parsedChanges ? (
        <JsonBlock
          label={t('common.auditLogs.detailChanges')}
          raw={JSON.stringify(parsedChanges, null, 2)}
        />
      ) : null}
              ...
              {related.map((r) => (
                <li key={r.id}>
                  ...
                  <Typography.Text type="secondary">
                    (
                    {parseAuditJsonField(r.metadata, 'targetUserId') ??
                      r.entityName ??
                      r.entityType}
                    )
                  </Typography.Text>
