  ...
  const runCleanup = useCallback(async () => {
    ...
    try {
      const res = await AXIOS_INSTANCE.delete<{
        ...
        message?: string;
      }>('/api/AuditLog/cleanup', { data: { cutoffDate: cleanupDate.format('YYYY-MM-DD') } });
      message.success(
    ...
    <Card size="small" title={t('common.auditLogs.retentionCardTitle')}>
      <Space orientation="vertical" style={{ width: '100%' }} size="middle">
        ...
        <Form form={holdForm} layout="inline">
          ...
          >
            <DatePicker
              placeholder={t('common.auditLogs.legalHoldFrom')}
              format={DAYJS_DATE_FORMAT}
            />
          </Form.Item>
          <Form.Item
          ...
          >
            <DatePicker
              placeholder={t('common.auditLogs.legalHoldTo')}
              format={DAYJS_DATE_FORMAT}
            />
          </Form.Item>
