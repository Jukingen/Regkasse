...
export function isRksvFinanzOnlineTrackedSpecialReceiptKind(
...
export function rksvFinanzOnlineSubmissionStatusTagColor(
...
export function shouldOfferFinanzOnlineReconciliationRetry(
  status: string | null | undefined
): boolean {
  const s = (status ?? '').trim();
