...
import type { CashRegister, MonatsbelegStatusDto } from '@/api/generated/model';
import {
...
export function useAdminMonatsbelegOverview(enabled = true) {
  ...
  const registerIds = useMemo(
    () =>
      registers.map((r) => r.id?.trim()).filter((id): id is string => Boolean(id && id.length > 0)),
    [registers]
  );
  ...
  const redCount = useMemo(
    () => rows.filter((r) => r.status?.missingMonths?.some((m) => m.isOverdue)).length,
    [rows]
  );
