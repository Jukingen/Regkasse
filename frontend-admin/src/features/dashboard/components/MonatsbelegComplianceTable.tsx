...
function getYearlyProgress(missingMonths: MissingMonth[] | undefined): {
  completed: number;
  total: number;
  percent: number;
} {
...
export function MonatsbelegComplianceTable({
  ...
  const otherMissingCount = useMemo(
    () => countPastMissingMonatsbelege(overviewItems),
    [overviewItems]
  );
  const pastMissingEntries = useMemo(
    () => collectPastMissingMonatsbelege(overviewItems),
    [overviewItems]
  );

  const createCurrentMonthMonatsbelegForTest = useCallback(
    async (record: RegisterMonatsbelegRow) => {
  ...
  const columns: ColumnsType<RegisterMonatsbelegRow> = useMemo(
    () => [
      {
        ...
        render: (_, record) => (
          <Space orientation="vertical" size={0}>
            <Typography.Text strong>{record.register.location?.trim() || '—'}</Typography.Text>
      ...
      },
      {
        ...
        render: (_, record) => {
          if (record.statusLoading)
            return <Tag color="default">{t('dashboard.monatsbeleg.loadingTag')}</Tag>;
          if (record.statusError)
            return <Tag color="default">{t('dashboard.monatsbeleg.statusFailed')}</Tag>;
          return (
            <Typography.Text>
    ...
    [
      canMonatsbeleg,
      canOpenSonderbelege,
      createCurrentMonthMonatsbelegForTest,
      demoAutoSuggestEnabled,
      t,
    ]
  );
