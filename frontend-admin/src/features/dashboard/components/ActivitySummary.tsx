...
function getActivityTypeTagColor(type: string, severity: ActivitySeverity): string {
  ...
  const colors: Record<string, string> = {
    ...
    CashRegisterOpenRequested: 'gold',
    CashRegisterOpenRequestApproved: 'green',
    CashRegisterOpenRequestDenied: 'default',
    ...
    MonatsbelegMissingReminder: 'gold',
    ...
    DailyClosingOpenOrdersWarning: 'gold',
    OnlineOrderPushedToPos: 'blue',
    OnlineOrderPaid: 'green',
          ...
          <List.Item style={{ padding: '8px 0' }}>
            <Space orientation="vertical" size={2} style={{ width: '100%' }}>
              <Space wrap>
                ...
                {item.actorName ? <Typography.Text strong>{item.actorName}</Typography.Text> : null}
              </Space>
