...
export function RksvReminderStatusCard({ enabled = true }: RksvReminderStatusCardProps) {
  ...
  const { rows, summary, registersLoading, statusPending, hasRegisters, loadError } =
    useRksvReminderOverview(enabled);
    ...
    >
      ...
      {loadError ? (
        <Alert
          type="error"
          showIcon
          title={t('dashboard.rksvReminder.status_load_failed')}
      ...
      {!loadError && !hasRegisters && !loading ? (
        <Typography.Text type="secondary">
          {t('dashboard.rksvReminder.no_registers')}
        </Typography.Text>
      ) : null}
