...
export function MonatsbelegWidget({ enabled = true }: MonatsbelegWidgetProps) {
  const { t } = useI18n();
  const { data, isLoading, error, refetch, hasRegisters, isFetching } =
    useMonatsbelegDashboard(enabled);
  ...
  const displayError =
    error ?? (loadTimedOut ? new Error(t('dashboard.monatsbeleg.loadTimeout')) : null);
    ...
    return (
      ...
      >
        <Alert
          ...
          description={displayError.message || t('dashboard.monatsbeleg.loadFailedDescription')}
          type="error"
          showIcon
        />
      </Card>
