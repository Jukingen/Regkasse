...
function mapSessionPolicy(raw: SessionPolicyResponse | undefined) {
  ...
  const timeout = raw.sessionTimeoutMinutes ?? raw.SessionTimeoutMinutes;
  const warning = raw.warningBeforeTimeoutMinutes ?? raw.WarningBeforeTimeoutMinutes;
