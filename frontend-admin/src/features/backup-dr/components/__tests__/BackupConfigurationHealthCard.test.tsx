...
describe('BackupConfigurationHealthCard', () => {
  it('renders adapter, worker, archive, verification and issues', () => {
    ...
    expect(
      screen.getByText('backupDr.monitoring.configHealth.externalArchiveNotConfigured')
    ).toBeInTheDocument();
    expect(screen.getByText('missing archive')).toBeInTheDocument();
    expect(
      screen.getByRole('link', { name: 'backupDr.monitoring.configHealth.editSettings' })
    ).toHaveAttribute('href', '/backup/configuration#backup-dr-schedule-settings');
  });
  ...
  it('shows PgDump adapter with success badge semantics via label', () => {
    ...
    expect(
      screen.getByText('backupDr.monitoring.configHealth.externalArchiveConfigured')
    ).toBeInTheDocument();
  });
});
