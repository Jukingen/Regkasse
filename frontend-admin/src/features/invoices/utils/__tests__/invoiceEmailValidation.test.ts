...
import { isValidInvoiceRecipientEmail, resolveInvoiceRecipientEmail } from '../invoiceEmailValidation';
...
describe('invoiceEmailValidation', () => {
    ...
    it('prefers typed input over stored customer email', () => {
        expect(resolveInvoiceRecipientEmail('  typed@example.com ', 'stored@example.com')).toBe('typed@example.com');
    });
