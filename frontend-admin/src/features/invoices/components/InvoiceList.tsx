...
function getPaymentMethodLabel(method: PaymentMethod | undefined, t: InvoiceTranslateFn): string {
  switch (method) {
    case 0:
      return t('invoices.paymentMethod.bar');
    case 1:
      return t('invoices.paymentMethod.card');
    case 2:
      return t('invoices.paymentMethod.transfer');
    case 3:
      return t('invoices.paymentMethod.check');
    case 4:
      return t('invoices.paymentMethod.voucher');
    case 5:
      return t('invoices.paymentMethod.mobile');
    default:
...
function invoiceListLocalizedErrorHint(err: unknown, t: InvoiceTranslateFn): string {
  ...
  if (
    ...
    /network|failed to fetch|load failed|econnrefused|timeout/i.test(err.message)
  ) {
...
export const InvoiceList: React.FC = () => {
  ...
  const {
    ...
    refetch,
  } = useQuery<InvoiceListItemDtoPagedResult, Error>({
  ...
  const handleCreateCreditNote = async () => {
    try {
      ...
      if (status === 409) {
        ...
        message.error(
          getAxiosResponseDataString(err) ?? t('invoices.messages.creditNoteBadRequest')
        );
        ...
        for (const key of selectedRowKeys) {
          try {
            ...
            if (res.success) {
              ...
              attemptedRows.push({
                cashRegisterId: parseAuthoritativeRegisterGuid(inv.cashRegisterId) ?? undefined,
  ...
  const handleSubmitFinanzOnline = async (invoice: Invoice) => {
    ...
    try {
      ...
      const invoiceDate = dayjs(invoice.invoiceDate).isValid()
      ...
      if (data.success) {
        ...
        modal.warning({
          ...
          content: (
            <Space orientation="vertical" size={8}>
              <Typography.Text>
                {data.message || t('invoices.reconciliation.retryUnknownError')}
              </Typography.Text>
              <Typography.Text type="secondary">
      ...
      if (selectedInvoiceId) {
        void queryClient.invalidateQueries({
          queryKey: getGetApiInvoiceIdQueryKey(selectedInvoiceId),
        });
      }
      void invalidateReconciliationViews();
    } catch (err: unknown) {
      ...
      modal.error({
        ...
        content: (
          <Space orientation="vertical" size={8}>
            <Typography.Text>
              {getAxiosResponseDataString(err) ?? t('invoices.reconciliation.catchFallbackMessage')}
            </Typography.Text>
            <Typography.Text type="secondary">
      ...
      render: (text: string | null | undefined, record) => {
        ...
        return (
          <Space size={4} wrap align="center">
            ...
            >
              <Typography.Text type="secondary">
                {text?.trim() || FORMAT_EMPTY_DISPLAY}
              </Typography.Text>
            </Tooltip>
  ...
  const displayInvoiceNumber =
    ...
    selectedInvoiceId ||
    t('invoices.display.unknownInvoice');

  const tableEmptyText = isInitialListLoading ? undefined : dateRangeError ? (
    ...
    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t('invoices.empty.loadFailedTitle')}>
      ...
      <BackendRawTextBlock
        ...
        body={extractRawApiErrorMessage(listQueryError)}
      />
      ...
      <Space orientation="vertical" size="large" style={{ width: '100%' }}>
        ...
        <Space orientation="vertical" style={{ width: '100%' }} size="middle">
          ...
          <Row gutter={[16, 16]}>
            ...
            <Col xs={24} sm={8} md={5}>
              <Space orientation="vertical" size={4} style={{ width: '100%' }}>
                <Input
                  ...
                  onChange={(e) => {
                    setCashRegisterIdFilter(e.target.value || undefined);
                    setPagination((p) => ({ ...p, current: 1 }));
                  }}
                  allowClear
                />
            ...
            <Col xs={24} sm={8} md={24} lg={2} style={{ textAlign: 'right' }}>
          ...
          {hasActiveFilters ? (
            <div>
              <Space wrap size={[8, 8]} align="center">
                ...
                {invalidRegisterOnly ? (
                  <Tag closable onClose={() => setInvalidRegisterOnly(false)} color="purple">
          ...
          {/* Table */}
          <VirtualTable<InvoiceListItemDto>
            rowSelection={{
              selectedRowKeys,
        ...
        {detailLoading ? (
          ...
          <React.Fragment>
            {(() => {
              ...
              const detailDate = dayjs(
                detailInvoice.invoiceDate || detailInvoice.createdAt
              ).isValid()
                ...
                <Space orientation="vertical" size={0} style={{ width: '100%' }}>
                  <OperatorSummaryStrip>
                    <Space wrap size={[16, 12]} align="start">
                      ...
                      <div>
                        ...
                        <Tag color={invoiceStatusMap[detailInvoice.status]?.color || 'default'}>
                          {invoiceStatusMap[detailInvoice.status]?.label ||
                            displayScalar(detailInvoice.status)}
                        </Tag>
                      </div>
                      <div>
                        <Typography.Text
                    ...
                    <Space wrap align="center" size={8}>
                      ...
                      <AdminTruthBadge kind={registerDeepLinkEligibleBadgeKind(detailRegFk)} />
                      ...
                      {detailRegFk.linkSafeUuid ? (
                        <Link
                          href={buildFinanzOnlineQueueInvestigationHref({
                            ...
                            investigationBatchCorrelationId: detailCorrelationTrimmed || undefined,
                          })}
                          prefetch={false}
                          target="_blank"
                    ...
                    <Space orientation="vertical" size={6} style={{ width: '100%' }}>
                      <Space wrap align="center">
                        <Typography.Text type="secondary">
                          {t('invoices.detail.paymentReconciliation')}
                        </Typography.Text>
                        {detailInvoice.sourcePaymentId ? (
                          ...
                          <Typography.Text type="secondary">
                            {t('invoices.detail.noPaymentLinked')}
                          </Typography.Text>
                        )}
                      </Space>
                      <Space wrap align="center">
                    ...
                    >
                      {provenanceFacet.kind === 'explicit_backend_string' ? (
                        ...
                        <>
                          <strong>{t('invoices.detail.provenancePlain')}</strong>{' '}
                          {t('invoices.detail.provenanceOperatorFooter')}
                        </>
                      )}
                    </Typography.Paragraph>
                    {(detailInvoice.status === InvoiceStatus.NUMBER_1 ||
                  ...
                  <OperatorBusinessSection>
                    <Descriptions bordered column={2} size="small">
                      ...
                      <Descriptions.Item
                        label={t('invoices.detail.descRegisterFkMachine')}
                        span={2}
                      >
                      ...
                      <Descriptions.Item
                        label={t('invoices.detail.descLabelPaymentMethod')}
                        span={2}
                      >
                  ...
                  <OperatorTechnicalSection>
                    <Collapse
                      ...
                      items={[
                        ...
                        {
                          key: 'items',
                          label: t('invoices.detail.positionsCollapseLabel', {
      ...
      >
        ...
        <Form form={creditNoteForm} layout="vertical">
          ...
          >
            <Select placeholder={t('invoices.creditNote.formReasonPlaceholder')}>
              ...
              <Select.Option value="DISCOUNT">
                {t('invoices.creditNote.reasonDiscount')}
              </Select.Option>
          ...
          >
            <Input.TextArea
              rows={3}
              placeholder={t('invoices.creditNote.formReasonTextAreaPlaceholder')}
            />
          </Form.Item>
        </Form>
      </Modal>
