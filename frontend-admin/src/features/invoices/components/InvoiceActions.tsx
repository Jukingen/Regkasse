...
import { isValidInvoiceRecipientEmail, resolveInvoiceRecipientEmail } from '../utils/invoiceEmailValidation';
...
function buildStatusMap(t: (key: string) => string): Record<number, { label: string; color: string }> {
    return {
...
export const InvoiceActions: React.FC<InvoiceActionsProps> = ({ invoice, size = 'middle', onSuccess }) => {
    ...
    const statusInfo = statusMap[statusCode] ?? { label: t('invoices.status.unknown'), color: 'default' };
    ...
    return (
        <>
            ...
            >
                {previewLoading ? (
                    <div style={{ textAlign: 'center', padding: 24 }}>{t('invoices.detail.pdfPreviewLoading')}</div>
                ) : pdfUrl ? (
                    <iframe
                    ...
                    <div style={{ textAlign: 'center', padding: 24 }}>{t('invoices.detail.pdfPreviewFailed')}</div>
                )}
            </Modal>
            ...
            >
                ...
                <div>
                    <div style={{ fontWeight: 500, marginBottom: 8 }}>{t('invoices.resend.formEmailLabel')}</div>
                    <Input
                        type="email"
