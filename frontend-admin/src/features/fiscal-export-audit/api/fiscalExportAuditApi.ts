...
export async function fetchFiscalExportAuditLogs(
  ...
  const { data } = await AXIOS_INSTANCE.get<FiscalExportAuditLogsPagedResponse>(
    '/api/admin/audit/fiscal-export-logs',
    {
      params: {
...
export async function fetchFiscalExportAuditDetail(
  id: string
): Promise<FiscalExportAuditLogDetail> {
  const { data } = await AXIOS_INSTANCE.get<FiscalExportAuditLogDetail>(
    `/api/admin/audit/fiscal-export-logs/${id}`
  );
  return data;
}

export function buildFiscalExportAuditCsvExportUrl(
  params: FiscalExportAuditListParams & { maxRows?: number }
): string {
