...
function DependencyOverviewBody({
  ...
  const hardDeleteControl = !canHardDelete ? (
  ...
  const columns: ColumnsType<PreparationTableRow> = [
    ...
    },
    {
      ...
      render: (status: PreparationTableRow['status']) => {
        if (status === 'blocking') {
          return <Tag color="error">{t('tenants.deletePreparation.statusBlocking')}</Tag>;
        }
        if (status === 'compliance') {
          return <Tag color="warning">{t('tenants.deletePreparation.statusCompliance')}</Tag>;
        }
        if (status === 'present') {
