...
import { fiscalExportDisclaimerAckHeaders } from '@/api/admin-rksv/client';
import {
...
function DecommissionWizardStep2({
  ...
  return (
    <Space orientation="vertical" size="large" style={{ width: '100%' }}>
      {!canDecommissionCashRegisters ? (
        <Alert type="error" showIcon title={t('tenants.decommission.registers.permissionDenied')} />
      ) : null}
      ...
      <Space wrap>
        <Button onClick={onPrevious}>{t('tenants.decommission.actions.back')}</Button>
        <Button
          danger
          type="primary"
    ...
    <Space orientation="vertical" size="large" style={{ width: '100%' }}>
      ...
      {fiscalExportPending ? (
        <Progress
          ...
          status="active"
          format={(percent) => t('tenants.decommission.export.progress', { percent: percent ?? 0 })}
        />
      ) : null}
            ...
            <Space orientation="vertical" size="small">
              ...
              <Typography.Text type="secondary">
                {t('tenants.decommission.export.readyAt', {
                  value: lastExportedAtUtc ? formatDateTime(lastExportedAtUtc, formatLocale) : '—',
                })}
              </Typography.Text>
            </Space>
          }
      ...
      <Space wrap>
        ...
        <Button onClick={onNext} disabled={!canAdvanceFromFiscalExport || fiscalExportPending}>
    ...
    <Result
      ...
      extra={[
        <Link href="/admin/tenants" key="list">
          <Button type="primary">{t('tenants.decommission.actions.backToList')}</Button>
        </Link>,
      ...
      <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
        <Typography.Text type="secondary">
          {t('tenants.decommission.result.archivedAt', {
            value: archivedAtUtc ? formatDateTime(archivedAtUtc, formatLocale) : '—',
          })}
        </Typography.Text>
        ...
        <Space wrap>
          <Link href={auditHref}>{t('tenants.decommission.actions.openAuditLogs')}</Link>
          <Typography.Text type="secondary">
...
export function TenantDecommissionWizardPage() {
  ...
  const canAccess = isSuperAdmin(user?.role) || hasPermission(PERMISSIONS.SYSTEM_CRITICAL);
  ...
  const eligibleRegisters = useMemo(
    () =>
      registers.filter((register) =>
        canDecommissionRegister(typeof register.status === 'number' ? register.status : undefined)
      ),
    [registers]
  );
  ...
  const fiscalExportProfile = canExportCompliancePackage
    ...
    : canExportAuditPackage
      ? 'accounting_report'
      : 'diagnostic_package';
  const fiscalExportFromUtc =
    tenant?.createdAt ?? new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString();
  ...
  const bulkDecommissionMutation = useMutation({
    ...
    onSuccess: async (results) => {
      ...
      message.success(
        t('tenants.decommission.messages.registersDecommissioned', { count: results.length })
      );
  ...
  const fiscalExportMutation = useMutation({
    mutationFn: async () => {
      const exportableRegisters = registers.filter(
        (register) => typeof register.id === 'string' && register.id.length > 0
      );
      ...
      for (let index = 0; index < exportableRegisters.length; index += 1) {
        ...
        if (
          !isDeferredFiscalExportResponse(generateResponse) ||
          typeof generateResponse.exportId !== 'string'
        ) {
        ...
        zip.file(`${tenantSlug}/${registerNumber}-${periodFrom}-${periodTo}.json`, blob);
    ...
    () => [
      ...
      },
      {
        ...
        render: (status?: number) => (
          <Tag color={registerStatusTagColor(status)}>{formatRegisterStatus(status, t)}</Tag>
        ),
      },
      {
  ...
  const handleDecommissionAll = useCallback(() => {
    bulkDecommissionMutation.mutate({
      ...
      reason: decommissionReason.trim() || t('tenants.decommission.registers.defaultReason'),
    });
  ...
  return (
    <AdminPageShell>
      ...
      <Space orientation="vertical" size="large" style={{ width: '100%' }}>
        ...
        {tenantQuery.isError ? (
          <Alert type="error" showIcon title={t('tenants.users.errors.tenantNotFound')} />
        ) : null}

        {registersQuery.isError ? (
          <Alert type="error" showIcon title={t('tenants.decommission.registers.loadFailed')} />
        ) : null}
