...
import { Alert, Button, Card, Checkbox, Input, Modal, Space, Table, Tag, Typography } from 'antd';
...
export function TenantPermanentDeleteModal({
  ...
  const slugMatches = confirmSlug.trim().toLowerCase() === tenantSlug.trim().toLowerCase();
  const phraseMatches = confirmPhrase.trim() === TENANT_PERMANENT_DELETE_CONFIRM_PHRASE;
  ...
  const submitBlockedMessage =
    submitFailureCode || submitSummary
      ? resolveTenantDeleteFailureMessage(
          t,
  ...
  const columns: ColumnsType<DependencyTableRow> = [
    ...
    },
    {
      ...
      render: (status: DependencyTableRow['status']) => {
        if (status === 'blocking') {
          return <Tag color="error">{t('tenants.permanentDeleteModal.status.blocking')}</Tag>;
        }
        if (status === 'compliance') {
          return <Tag color="warning">{t('tenants.permanentDeleteModal.status.compliance')}</Tag>;
        }
              ...
              {blockingDependencies.map((blocker) => {
                ...
                return (
                  <Tag
                    ...
                    color={blocker.severity === 'compliance' ? 'warning' : 'error'}
                  >
                    {label}
            ...
            <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
              ...
              <div>
                ...
                <Input
                  ...
                  autoComplete="off"
                  status={confirmPhrase.length > 0 && !phraseMatches ? 'error' : undefined}
                  style={{ marginTop: 8 }}
                />
              </div>
              <Checkbox checked={retentionAck} onChange={(e) => setRetentionAck(e.target.checked)}>
