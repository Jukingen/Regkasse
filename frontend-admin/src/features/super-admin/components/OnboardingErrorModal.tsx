...
export function OnboardingErrorModal({
  ...
  const suggestionsQuery = useQuery({
    ...
    enabled:
      open && !!error && (error.slugSuggestions.length === 0 || error.code === 'tenant_slug_taken'),
    staleTime: 30_000,
  });
          ...
          {suggestions.length > 0 ? (
            <div>
              <Typography.Text strong>
                {t('tenants.onboarding.errors.suggestionsTitle')}
              </Typography.Text>
              <List
                size="small"
                style={{ marginTop: 8 }}
                ...
                renderItem={(slug) => (
                  <List.Item
                    actions={[
                      <Button key="use" type="link" size="small" onClick={() => onTrySlug(slug)}>
