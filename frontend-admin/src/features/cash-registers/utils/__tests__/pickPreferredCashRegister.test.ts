...
} from '@/features/cash-registers/utils/pickPreferredCashRegister';
...
describe('pickPreferredCashRegisterId', () => {
  ...
  it('keeps the current selection when it belongs to the tenant', () => {
    expect(pickPreferredCashRegisterId([regDefault, regOther], 'reg-other', tenantA)).toBe(
      'reg-other'
    );
  });

  it('prefers the tenant default when the stored id belongs to another tenant', () => {
    expect(pickPreferredCashRegisterId([regDefault, regOther], 'reg-foreign', tenantA)).toBe(
      'reg-default'
    );
  });
  ...
  it('scopes by tenant when tenantId is provided', () => {
    expect(pickPreferredCashRegisterId([regDefault, regForeign], null, tenantA)).toBe(
      'reg-default'
    );
  });
});
