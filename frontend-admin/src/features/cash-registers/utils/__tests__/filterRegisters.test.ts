...
describe('filterVisible registers', () => {
  it('hides decommissioned by default', () => {
    const rows = [{ id: '1', status: 2 } as CashRegister, { id: '2', status: 5 } as CashRegister];
    expect(filterCashRegisters(rows, { showDecommissioned: false }).map((r) => r.id)).toEqual([
      '1',
    ]);
  });

  it('shows all when checkbox enabled', () => {
    const rows = [{ id: '1', status: 1 } as CashRegister, { id: '2', status: 5 } as CashRegister];
