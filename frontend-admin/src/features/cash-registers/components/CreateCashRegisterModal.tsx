...
function CreateCashRegisterModalContent({
  ...
  const { tenants, isLoading: tenantsLoading } = useTenantList({
    enabled: visible && isSuperAdminUser,
  });
  ...
  const handleSubmit = async (values: CreateCashRegisterFormValues) => {
    try {
      await createMutation.mutateAsync({
        ...
        tenantId: isSuperAdminUser ? (values.tenantId ?? fixedTenantId ?? undefined) : undefined,
      });
    } catch (err) {
      message.error(
      ...
      <Form form={form} layout="vertical" onFinish={handleSubmit} requiredMark="optional">
        ...
        {showTenantPicker ? (
          <Form.Item
            ...
            rules={[{ required: true, message: t('cashRegisters.create.tenantRequired') }]}
          >
            <Select
