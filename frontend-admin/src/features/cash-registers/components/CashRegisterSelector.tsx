...
function resolveAutoSelectedRegisterId(
  ...
  if (
    saved &&
    registers.some((row) => row.id === saved && (!row.tenantId || row.tenantId === tenantId))
  ) {
    return saved;
  }
...
export function CashRegisterSelector({
  ...
  const filterRegisterOption = useCallback(
    (input: string, option?: { register?: AdminCashRegisterListItem }) => {
  ...
  const renderRegisterOption = useCallback(
    (register: AdminCashRegisterListItem) => {
      const mandant =
        listAllTenants && register.tenantId ? tenantById.get(register.tenantId) : null;
