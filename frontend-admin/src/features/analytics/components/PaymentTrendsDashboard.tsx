...
import { DAYJS_DATE_FORMAT, formatUserMonthDay, formatUserMonthYear } from '@/lib/dateFormatter';
...
export function PaymentTrendsDashboard() {
  ...
  const chartData = useMemo(
    () =>
      (trends?.trendData ?? []).map((point) => ({
        ...
        xLabel:
          point.label ??
          (period === 'Monthly' ? formatUserMonthYear(point.date) : formatUserMonthDay(point.date)),
      })),
  ...
  return (
    <div className={styles.dashboard}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              ...
              styles={{
                content: {
                  color: growthPositive ? '#3f8600' : growthNegative ? '#cf1322' : undefined,
                },
              }}
            />
            <div className={styles.growthHint}>
              {t('payments.trendsDashboard.vsPreviousPeriod')}
            </div>
          </Card>
        </Col>
      </Row>
      ...
      <Card
        title={t('payments.trendsDashboard.transactionVolumeTitle')}
            ...
            <div className={styles.methodList}>
              {methodPieData.map((method) => (
                <div key={method.method} className={styles.methodRow}>
                  ...
                  <span className={styles.methodPct}>({method.percentage.toFixed(1)}%)</span>
                </div>
              ))}
            </div>
          </Col>
