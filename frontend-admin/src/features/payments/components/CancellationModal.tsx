...
import {
  type AdminCancelPaymentRequest,
  cancelAdminPayment,
} from '@/features/payments/api/adminPaymentCancel';
import {
...
function CancellationModalContent({
  ...
  const executeCancel = async (payload: AdminCancelPaymentRequest) => {
    ...
    if (response.requiresApproval && !response.success) {
      ...
      setApprovalMessage(
        response.message ?? t('payments.cancellationModal.approvalRequiredFallback')
      );
          ...
          <Form layout="vertical">
            ...
            >
              <Input.OTP
                length={6}
                value={approvalToken}
                onChange={(value) => setApprovalToken(value.replace(/\D/g, '').slice(0, 6))}
