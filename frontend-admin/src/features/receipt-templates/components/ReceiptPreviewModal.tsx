...
export default function ReceiptPreviewModal({ templateId, onClose }: ReceiptPreviewModalProps) {
  ...
  return (
    <Modal
      title={
        data?.templateName
          ? t('receiptTemplates.preview.titleWithName', { name: data.templateName })
          : t('receiptTemplates.preview.title')
      }
      open={!!templateId}
      onCancel={onClose}
      footer={null}
    ...
    >
      ...
      {isError && (
        <Alert
          ...
          title={
            notFound
              ? t('receiptTemplates.preview.errorNotFound')
              : t('receiptTemplates.preview.errorGeneric')
          }
