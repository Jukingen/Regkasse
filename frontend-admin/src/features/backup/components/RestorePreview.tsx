...
export function RestorePreview({
  ...
  const { data: preview, isLoading, isError } = useRestorePreview(backup.id, {
    ...
    () => [
      ...
      },
      {
        ...
        render: (_: unknown, row: RestorePreviewChangeRow) => {
          const label = t(
            `backupDr.manualRestore.restorePreview.changeKind.${row.changeKind}`,
          );
  ...
  const complianceStatusTag = (() => {
    if (compliance.isLoading) {
      return (
        <Tag>{t("backupDr.manualRestore.restorePreview.compliance.status.checking")}</Tag>
      );
    }
  ...
  return (
    <Card size={size} title={t("backupDr.manualRestore.restorePreview.cardTitle")}>
      ...
      <Descriptions bordered size="small" column={1} style={{ marginBottom: 16 }}>
        ...
        <Descriptions.Item label={t("backupDr.manualRestore.restorePreview.labels.complianceStatus")}>
          {complianceStatusTag}
        </Descriptions.Item>
      </Descriptions>
      ...
      {preview ? (
        <>
          ...
          <Typography.Paragraph type="secondary" style={{ marginTop: 12, marginBottom: 8, fontSize: 12 }}>
