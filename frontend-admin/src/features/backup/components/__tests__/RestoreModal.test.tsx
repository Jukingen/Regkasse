...
vi.mock('@/features/backup/components/RestorePreview', () => ({
  RestorePreview: ({ onComplianceChange }: { onComplianceChange?: (ok: boolean) => void }) => {
    React.useEffect(() => {
...
describe('RestoreModal', () => {
  ...
  it('keeps restore disabled until both RKSV acknowledgements are checked', () => {
    ...
    fireEvent.click(screen.getByLabelText('backupDr.manualRestore.acknowledgements.sameTenant'));
    expect(restoreBtn).toBeDisabled();
