 ...
 * product_{tenantSlug}_{yyyyMMdd_HHmmss}.{csv|json}
 */
...
export function buildProductExportFileName(
  ...
  const slug = sanitizeProductExportFileSegment(tenantSlug, 'tenant');
  const stamp = formatLocalStamp(at);
  const ext = format?.trim().toLowerCase() === 'json' ? 'json' : 'csv';
  return `product_${slug}_${stamp}.${ext}`;
}

export function sanitizeProductExportFileSegment(
