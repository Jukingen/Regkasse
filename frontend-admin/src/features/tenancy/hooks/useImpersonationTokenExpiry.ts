...
export function computeImpersonationTokenExpiryState(token: string | null): ImpersonationTokenExpiryState {
    if (!token) {
...
export function useImpersonationTokenExpiry(enabled: boolean): ImpersonationTokenExpiryState {
    const [state, setState] = useState<ImpersonationTokenExpiryState>(() =>
        enabled ? computeImpersonationTokenExpiryState(authStorage.getToken()) : { expiresAtMs: null, minutesRemaining: null, shouldWarn: false },
    );
