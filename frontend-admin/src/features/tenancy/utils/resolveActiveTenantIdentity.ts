...
export function tenantSlugsMatch(
    ...
    return (
        canonicalDevTenantSlug(left).toLowerCase() === canonicalDevTenantSlug(right).toLowerCase()
    );
}
...
export function resolveActiveTenantId(input: {
    ...
    if (
        input.jwtTenantId?.trim()
        && tenantSlugsMatch(input.jwtTenantSlug, input.activeTenantSlug)
    ) {
