  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, record: Customer) => {
        const items = getBenefitSummaryForCustomer(
          record.id,
          benefitAssignments,
          t('customers.list.benefitFallback')
        );
    ...
    },
    {
      ...
      render: (_: unknown, record: Customer) => (
        <Space>
          {record.isActive ? (
            <Tag color="green">{t('customers.list.statusActive')}</Tag>
          ) : (
            <Tag color="red">{t('customers.list.statusInactive')}</Tag>
          )}
          ...
          <Space>
            ...
            {onManageBenefits && (
              <Tooltip title={t('customers.list.manageBenefits')}>
                <Button icon={<GiftOutlined />} onClick={() => onManageBenefits(record)} />
              </Tooltip>
            )}
            <Popconfirm
