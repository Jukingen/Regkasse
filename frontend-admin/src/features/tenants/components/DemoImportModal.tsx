...
function extractImportErrorMessage(error: unknown): string {
  if (error && typeof error === 'object' && 'response' in error) {
    const data = (
      error as { response?: { data?: { message?: string; error?: string; errorMessage?: string } } }
    ).response?.data;
    return (
      data?.message ??
      data?.error ??
      data?.errorMessage ??
      'Fehler beim Importieren der Demo-Produkte'
    );
  }
  return 'Fehler beim Importieren der Demo-Produkte';
}

export function DemoImportModal({
  open,
  tenantId,
  tenantName,
  tenantSlug,
  ...
  return (
    <>
      <Modal
        ...
        width={520}
        footer={
          <Button danger onClick={() => importAbortRef.current?.abort()}>
            Abbrechen
          </Button>
        }
      >
      ...
      <Modal
        ...
        onCancel={onClose}
        width={820}
        footer={activeTab === 'template' ? <Button onClick={onClose}>Schließen</Button> : null}
      >
        <Alert
