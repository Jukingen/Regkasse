...
export function applyDemoImportProfile(
  ...
  const groupNames = new Set(profile.groupNames.filter((name) => validGroupNames.has(name)));

  let productIds: Set<string>;
  if (profile.selectedProductIds?.length) {
    const catalogIds = new Set(catalogProducts.map((p) => p.id));
    productIds = new Set(profile.selectedProductIds.filter((id) => catalogIds.has(id)));
  } else {
