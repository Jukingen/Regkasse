...
export function DemoImportCatalogWizard({
  ...
  const categoryGroups = useMemo(() => {
    const countByName = new Map(catalog?.categories.map((c) => [c.name, c.productCount]) ?? []);
  ...
  const canGoNext = (): boolean => {
    switch (currentStep) {
      ...
      case 3:
        return selectedProducts.every((p) => isValidTaxRate(resolveWizardTax(p, taxOverrides)));
      default:
        return true;
    }
  };
      ...
      {currentStep === 2 ? (
        <Space orientation="vertical" style={{ width: '100%' }}>
          ...
          <Table
            ...
            pagination={
              selectedProducts.length > 50 ? { pageSize: 50, showSizeChanger: false } : false
            }
            scroll={{ y: 240 }}
          />
          ...
          <Alert
            ...
            description={
              <>
                ...
                <strong>{selectedGroupNames.size}</strong> Kategoriegruppen importiert (Gesamtwert{' '}
