...
export function MonatsbelegTimeline({
  ...
  const [receiptsMonth, setReceiptsMonth] = useState<{ year: number; month: number } | null>(null);
        ...
        title={
          summaryMonth
            ? t('rksvHub.monatsbelegTimeline.summaryTitle', {
                period: formatPeriodLabel(summaryMonth.year, summaryMonth.month, textLocale),
              })
        ...
        {summaryMonth && summaryPayload ? (
          <Spin spinning={receiptsQuery.isLoading || receiptsQuery.isFetching}>
            <Descriptions bordered size="small" column={1}>
              ...
              <Descriptions.Item label={t('rksvHub.monatsbelegTimeline.summaryTransactions')}>
            ...
            <Space wrap style={{ marginTop: 16 }}>
              {summaryMonth.status === 'missing' ? (
                <Button type="primary" onClick={() => handleAction('create-late', summaryPayload)}>
                  {t('rksvHub.monatsbelegTimeline.summaryCreate')}
                </Button>
              ) : null}
              {summaryMonth.status === 'completed' ? (
                <Button type="primary" onClick={() => handleAction('view-report', summaryPayload)}>
              ...
              {summaryMonth.status === 'completed' && canRecreate ? (
                <Button danger onClick={() => handleAction('recreate', summaryPayload)}>
        ...
        title={
          receiptsMonth
            ? t('rksvHub.monatsbelegTimeline.receiptsModalTitle', {
                period: formatPeriodLabel(receiptsMonth.year, receiptsMonth.month, textLocale),
              })
            ...
            <List
              ...
              renderItem={(receipt) => {
                ...
                return (
                      ...
                      id
                        ? [
                            <Link key="open" href={`/receipts/${id}`}>
                              {t('rksvHub.monatsbelegTimeline.receiptsModalOpenOne')}
                            </Link>,
                          ]
                        : undefined
                    }
                  >
                    <List.Item.Meta
                      title={id ? <Link href={`/receipts/${id}`}>{number}</Link> : number}
                      description={
