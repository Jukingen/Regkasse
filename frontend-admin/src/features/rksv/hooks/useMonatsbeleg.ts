 ...
 */
export function useCashRegisterMonatsbeleg(cashRegisterId: string, options?: MonatsbelegQueryOptions) {
    ...
    return useQuery<MonatsbelegStatusDto, unknown>({
        ...
        queryFn: ({ signal }) => getApiRksvMonatsbelegStatusCashRegisterId(trimmedId, undefined, signal),
        enabled,
        staleTime: MONATSBELEG_REFETCH_INTERVAL_MS,
        retry: monatsbelegQueryRetry,
        retryDelay: (attemptIndex: number) => 1000 * (attemptIndex + 1),
