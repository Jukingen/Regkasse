...
export function computeDepExportStats(exportResult: RksvDepExportRoot | null | undefined): DepExportStats | null {
    const groups = exportResult?.['Belege-Gruppe'];
    if (!groups?.length) {
        return exportResult ? { groupCount: 0, totalSignatures: 0, certificateThumbprints: [] } : null;
    }

    const totalSignatures = groups.reduce((sum, group) => sum + (group['Belege-kompakt']?.length ?? 0), 0);
