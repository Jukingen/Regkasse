 ...
 */
import { Alert, Button, DatePicker, Empty, Select, Space, Table, Tag, Typography } from 'antd';
...
function getActorDisplay(record: AuditEntry): string {
  const name =
    record.actorDisplayName != null && String(record.actorDisplayName).trim()
      ? String(record.actorDisplayName).trim()
      : null;
  const uid =
    record.userId != null && String(record.userId).trim() ? String(record.userId).trim() : null;
...
export function UserActivityTimeline({ userId, userName }: Props) {
  ...
  const { data, isLoading, isError, refetch } = useGetApiAuditLogUserUserId(validUserId, params, {
    query: {
  ...
  const openDiffModalWithFreshEntry = (record: AuditEntry) => {
    refetch()
      .then((result) => {
        const list = Array.isArray(result.data?.auditLogs)
          ? (result.data.auditLogs as AuditEntry[])
          : [];
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, record: AuditEntry) =>
        hasDiff(record) ? (
          <Button type="link" size="small" onClick={() => openDiffModalWithFreshEntry(record)}>
  ...
  return (
    <div>
      ...
      <Table
        ...
        locale={{
          emptyText: (
            <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t('users.activity.empty')} />
          ),
        }}
      />
