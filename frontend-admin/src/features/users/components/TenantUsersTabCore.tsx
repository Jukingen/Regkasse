...
export function TenantUsersTabCore({
  ...
  const listFetching = isFixedTenant
    ? fixedTenantUsersQuery.isFetching
    : scopedUsersQuery.isFetching;
  const listError = isFixedTenant ? fixedTenantUsersQuery.isError : scopedUsersQuery.isError;
  const refetchList = () =>
    isFixedTenant ? fixedTenantUsersQuery.refetch() : scopedUsersQuery.refetch();
    ...
    <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
      ...
      {isFixedTenant ? (
        <TenantUserTable
          ...
          onResetPassword={
            canProvision
              ? (userId) => {
                  const row = fixedTenantUsersQuery.data?.find((u) => u.userId === userId) ?? null;
                  setResetRow(row);
                }
              : undefined
          }
      ...
      {canProvision ? (
        <>
          ...
          <ResetPasswordModal
            open={!!resetRow}
            tenantId={
              tenantId ?? (resetRow && 'tenantId' in resetRow ? resetRow.tenantId : '') ?? ''
            }
            user={
              resetRow
                ? {
      ...
      {permissionsUser ? (
        <UserPermissionsModal
          ...
          userName={
            permissionsUser.name?.trim() ||
            permissionsUser.userName?.trim() ||
