...
} from '@/features/users/components/UserFormDrawer';
...
export default function UsersPage() {
  ...
  const [resetPasswordValidationError, setResetPasswordValidationError] = useState<string | null>(
    null
  );
  ...
  const { data: roles, isLoading: rolesLoading } = useRoles({
    enabled: policy.canView || !!editUserId,
  });
  const canManageRoles =
    policy.canCreateRole || policy.canDeleteRole || policy.canEditRolePermissions;
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateUserRequest }) =>
      gatewayUpdateUser(id, data),
  });
  const updateUserTenantsMutation = useMutation({
    mutationFn: ({ id, tenantIds }: { id: string; tenantIds: string[] }) =>
      updateUserTenants(id, tenantIds),
  });
  const resetPasswordMutation = useMutation({
    ...
    onError: (e: unknown) => {
      ...
      const fallback = is401
        ? t('users.messages.sessionExpiredOrUnauthorized')
        : is403
          ? t('users.messages.errorResetPasswordForbidden')
          : is404
            ? t('users.messages.errorResetPasswordUserNotFound')
      ...
      const firstBackendMessage =
        Array.isArray(fieldErrors) && fieldErrors.length > 0 ? fieldErrors[0] : normalized.message;
  ...
  const deactivateMutation = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) =>
      gatewayDeactivateUser(id, { reason }),
    onSuccess: (_data, { id }) => {
      ...
      <AdminPageHeader
        ...
        actions={
          <Space wrap>
            ...
            {isSuperAdminLayout ? <DevOrphanedUsersCleanupButton invalidatePlatformUsers /> : null}
          </Space>
        }
      >
      ...
      {editUserId ? (
        <UserFormDrawer
          ...
          loading={
            ...
            updateUserTenantsMutation.isPending ||
            ...
            onSuccess={() => {
              ...
              if (resetPasswordUser.id) {
                queryClient.invalidateQueries({
                  queryKey: [`/api/AuditLog/user/${resetPasswordUser.id}`],
                });
              }
            }}
          />
