...
export function BulkImportModal({ open, onClose, onSuccess }: Props) {
  const { message } = useAntdApp();
    ...
    const footer =
        step === 'upload'
            ? [
                  <Button key="template" icon={<DownloadOutlined />} onClick={() => void downloadBulkImportTemplate()}>
                      Vorlage (CSV)
                  </Button>,
        ...
        <>
            ...
            >
                <Typography.Paragraph type="secondary">
                    ...
                    Cashier, Accountant. Große Dateien werden im Hintergrund verarbeitet (1000+ Zeilen).
                ...
                {step === 'importing' ? (
                    <Space orientation="vertical" style={{ width: '100%' }} size="large">
                        ...
                        <Typography.Text type="secondary">
                            ...
                            {jobStatus?.successCount ?? 0} erfolgreich, {jobStatus?.failedCount ?? 0} fehlgeschlagen
                        </Typography.Text>
                    </Space>
                ) : null}
