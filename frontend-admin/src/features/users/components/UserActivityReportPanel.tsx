  ...
  const timelineColumns = [
    ...
    },
    {
      ...
      render: (action: string) => {
        ...
        const label = labelKey ? t(labelKey as 'common.auditLogs.actionLabels.login') : action;
    ...
    <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card size="small" loading={isLoading}>
            <Statistic
              ...
              value={data?.lastLoginAt ? formatDateTime(data.lastLoginAt, formatLocale) : NA}
            />
        ...
        <Col xs={24} sm={12} lg={4}>
          <Card size="small" loading={isLoading}>
            <Statistic
              ...
              value={
                data?.lastSessionEndAt ? formatDateTime(data.lastSessionEndAt, formatLocale) : NA
              }
            />
          </Card>
        </Col>
      ...
      <Card size="small" title={t('reporting.userActivity.actionSummary')} loading={isLoading}>
        <Row gutter={[16, 16]}>
          <Col xs={12} sm={8} md={4}>
            <Statistic
              title={t('reporting.userActivity.userCreates')}
              value={actions?.userCreates ?? 0}
            />
          </Col>
          <Col xs={12} sm={8} md={4}>
            <Statistic
              title={t('reporting.userActivity.userEdits')}
              value={actions?.userEdits ?? 0}
            />
          </Col>
          <Col xs={12} sm={8} md={4}>
            <Statistic
