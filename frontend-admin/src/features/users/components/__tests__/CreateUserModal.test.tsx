...
describe('CreateUserModal', () => {
  ...
  it('creates with the selected tenant in onSubmit without calling onAssignTenants', async () => {
    ...
    expect(
      screen.queryByText('Optional können Sie jetzt Mandanten für alice@example.com auswählen.')
  ...
  it('opens the assignment modal when no tenant was selected during creation', async () => {
    ...
    expect(
      await screen.findByText('Optional können Sie jetzt Mandanten für bob@example.com auswählen.')
    ).toBeInTheDocument();
  });
  ...
  it('uses deferred assignment in quick mode when no tenant is selected', async () => {
    ...
    expect(
      screen.getByText('Nach der Erstellung müssen Sie den Benutzer einem Mandanten zuordnen.')
    ).toBeInTheDocument();
    ...
    expect(
      await screen.findByText(
        'Optional können Sie jetzt Mandanten für manager_ab12cd@platform.regkasse.at auswählen.'
      )
    ).toBeInTheDocument();
  });
});
