...
import { type CreateQuickUserResult, createQuickUser } from '@/features/super-admin/api/quickUser';
...
import type {
  UnifiedAdminUserRow,
  UnifiedAdminUserType,
} from '@/features/users/types/unifiedAdminUserRow';
import {
...
const TENANT_ROLE_FILTER_VALUES = [
  'SuperAdmin',
  'Manager',
  'Cashier',
  'Accountant',
...
export function UnifiedAdminUsersView({
  ...
  const tenantFilter = isTenantScoped ? tenantScopeId! : tenantFilterFromUiValue(selectedTenant);
  ...
  const groupedRows = useMemo(() => {
    ...
    filteredRows.forEach((row) => {
      ...
      const groupKey = isPlatform
        ? PLATFORM_GROUP_KEY
        : row.tenantId || row.tenantSlug || row.tenantName || 'unknown';
  ...
  const isLoading =
    ...
    (tenantFilter !== FILTER_ALL && tenantFilter !== FILTER_PLATFORM && tenantUsersQuery.isLoading);

  const isFetching =
    allUsersQuery.isFetching || platformQuery.isFetching || tenantUsersQuery.isFetching;
  ...
  const renderActions = (row: UnifiedAdminUserRow) => {
    ...
    const canDeactivateUser =
      policy.canDeactivate && record.isActive && !isPlatformUserRole(row.role);
    ...
    () => [
      ...
      },
      {
        ...
        render: (role: string, row) => (
          <UserRoleBadge role={role} isOwner={row.kind === 'tenant' ? row.isOwner : false} />
        ),
      },
      {
      ...
      {canCreateTenantUsers ? (
        <>
          <CreateUserModal
            ...
            onAssignTenants={
              isSuperAdminActor
                ? (userId, tenantIds) => assignTenantsMutation.mutateAsync({ userId, tenantIds })
                : undefined
            }
            onSubmit={(values) =>
