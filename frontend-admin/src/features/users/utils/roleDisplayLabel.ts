...
export function isCanonicalRoleName(
  roleName: string
): roleName is (typeof CANONICAL_ROLE_NAMES)[number] {
...
export function formatRoleDisplayLabel(
  ...
  return isCanonicalRoleName(roleName) ? t(`users.roles.displayNames.${roleName}`) : roleName;
}
...
export function formatRoleBadgeLabel(
  ...
  return isCanonicalRoleName(roleName) ? t(`users.roles.badgeLabels.${roleName}`) : roleName;
}
