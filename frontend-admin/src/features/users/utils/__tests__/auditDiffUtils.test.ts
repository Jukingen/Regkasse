...
import { EMPTY_PLACEHOLDER, formatDiffValue, parseAuditDiff } from '../auditDiffUtils';
...
describe('formatDiffValue', () => {
  ...
  it('formats boolean as active/inactive with options', () => {
    expect(
      formatDiffValue('isActive', true, { labelActive: 'Aktiv', labelInactive: 'Inaktiv' })
    ).toBe('Aktiv');
    expect(
      formatDiffValue('isActive', false, { labelActive: 'Aktiv', labelInactive: 'Inaktiv' })
    ).toBe('Inaktiv');
  });
