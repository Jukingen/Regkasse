...
function LicenseStatusTag({ licenseStatus }: { licenseStatus: ResolvedLicenseStatus }) {
  ...
  return <Tooltip title={tooltip ? `${message} ${tooltip}` : message}>{content}</Tooltip>;
}
...
export function TenantInfoCard({ className, style }: TenantInfoCardProps) {
  ...
  const { jwtTenantSlug, isDevTenantOverride, isImpersonating, isPlatformAdminHost } =
    useTenantContext();
  const { tenantSlug, tenantId, tenantName, registeredAt, licenseStatus, hasAuthToken, isLoading } =
    useTenantInfo();
