...
export function resolveTenantGraceDays(input: {
    ...
    if (typeof input.gracePeriodRemaining === 'number' && Number.isFinite(input.gracePeriodRemaining)) {
    ...
    if (typeof input.daysRemaining === 'number' && Number.isFinite(input.daysRemaining) && input.daysRemaining < 0) {
