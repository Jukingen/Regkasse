...
import type { ReceiptReprintRequest } from '@/api/generated/model';
import type { ReceiptReprintResponse } from '@/api/generated/model';
...
export default function ReceiptReprintWizard({
    ...
    const deviceOptions = (routingOptionsQuery.data?.devices ?? []).map((d) => ({
        value: d.id ?? '',
        label: d.isSimulated === false ? (d.label ?? d.id) : `${d.label ?? d.id} (${t('adminShell.operationsCenter.reprintSimulatedTag')})`,
    }));
    ...
    const onConfirm = async () => {
        ...
        try {
            ...
            const data = await postApiAdminBackofficeReceiptsPaymentIdReprintRequest(effectivePaymentId.trim(), body);
            setResult(data);
            ...
            if (isAxiosError(e) && e.response?.data && typeof e.response.data === 'object') {
                ...
                setResult(payload);
                setSubmitError(payload.errorMessage ?? payload.errorCode ?? t('adminShell.operationsCenter.reprintFailedGeneric'));
        ...
        >
            ...
            {step === 2 ? (
                <Space direction="vertical" size="middle" style={{ width: '100%' }}>
                    ...
                    <Space wrap>
                        <Button onClick={() => setStep(1)}>{t('adminShell.operationsCenter.reprintBack')}</Button>
            ...
            {step === 3 && result ? (
                <Space direction="vertical" size="middle" style={{ width: '100%' }}>
                    ...
                    {result.receipt?.receiptId ? (
                        <Link href={`/receipts/${result.receipt.receiptId}`}>{t('adminShell.operationsCenter.openReceipt')}</Link>
                    ) : null}
                    <Space wrap>
