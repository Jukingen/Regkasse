...
export function UserActivityReport({ initialUserId }: Props) {
  ...
  const [filters, setFilters] = useState<FilterState | null>(
    initialUserId ? { userId: initialUserId, dateRange: defaultRange, actionType: '' } : null
  );
          ...
          <Row gutter={16}>
            <Col xs={24} md={10}>
              <Form.Item
                ...
                rules={[
                  { required: true, message: t('reporting.userActivity.selectUserRequired') },
                ]}
              >
                <Select
                  showSearch
        ...
        <>
          <Card size="small" loading={isLoading || isFetching}>
            <Row gutter={[16, 16]}>
              ...
              <Col xs={12} sm={8} md={4}>
                <Statistic
                  title={t('reporting.userActivity.totalLogins')}
                  value={report.totalLogins}
                />
              </Col>
              <Col xs={12} sm={8} md={4}>
                <Statistic
              ...
              <Col xs={24} sm={12} md={8}>
                <Statistic
                  ...
                  value={
                    report.lastLoginAt ? formatDateTime(report.lastLoginAt, formatLocale) : '—'
                  }
          ...
          {report.topActiveUsers.length > 0 && (
            <Card size="small" title={t('reporting.userActivity.topActiveUsers')}>
              <Table
                ...
                columns={[
                  {
                    title: t('reporting.userActivity.selectUser'),
                    dataIndex: 'userName',
                    key: 'userName',
                  },
...
function TimelineTable({
  ...
  const columns = [
    ...
    {
      title: t('reporting.userActivity.timeline.entityType'),
      dataIndex: 'entityType',
      key: 'entityType',
      width: 100,
    },
    {
      title: t('reporting.userActivity.timeline.status'),
      dataIndex: 'status',
      key: 'status',
      width: 90,
    },
    {
      title: t('reporting.userActivity.timeline.ip'),
      dataIndex: 'ipAddress',
      key: 'ip',
      width: 110,
    },
    {
      title: t('reporting.userActivity.timeline.session'),
      dataIndex: 'sessionId',
      key: 'session',
      ellipsis: true,
    },
    {
      title: t('reporting.userActivity.timeline.correlation'),
