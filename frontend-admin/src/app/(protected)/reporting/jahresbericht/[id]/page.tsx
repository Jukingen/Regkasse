...
export default function JahresberichtDetailPage() {
  ...
  const finalizeMut = useMutation({
    mutationFn: async () => AXIOS_INSTANCE.post('/api/reports/jahresbericht/finalize', { reportId: id, note: null }),
    onSuccess: () => {
  ...
  const submitMut = useMutation({
    mutationFn: async () => AXIOS_INSTANCE.post(`/api/reports/jahresbericht/${id}/submit-finanzonline`),
    onSuccess: () => {
  ...
  const linkedCols: ColumnsType<JahresberichtDto['summary']['linkedFinalizedMonatsberichte'][0]> = useMemo(
    () => [
      ...
      { title: 'Kasse', dataIndex: 'registerNumber', render: (v, r) => v ?? r.cashRegisterId ?? '—' },
      { title: 'Brutto', dataIndex: 'grossSalesAmount', render: (v: number) => v?.toFixed(2) },
      { title: 'Monatsbericht', key: 'link', render: (_, r) => <Link href={`/reporting/monatsbericht/${r.monatsberichtId}`}>Öffnen</Link> },
    ],
    [],
  );
  ...
  if (detailQ.isLoading) return <Typography.Paragraph>Laden…</Typography.Paragraph>;
  if (detailQ.isError || !d) return <Typography.Paragraph type="danger">Bericht nicht gefunden oder Fehler beim Laden.</Typography.Paragraph>;
  ...
  return (
    <div style={{ paddingBottom: 24 }}>
      <AdminPageHeader
        title={`Jahresbericht ${d.summary.viennaYear}`}
        breadcrumbs={[adminOverviewCrumb(t), { title: 'Jahresbericht', href: '/reporting/jahresbericht' }, { title: id }]}
        actions={
          <Space wrap>
            {canExport && d.reportStatus === 'Provisional' ? (
              <Button type="primary" loading={finalizeMut.isPending} onClick={() => finalizeMut.mutate()}>Finalisieren</Button>
            ) : null}
            {canSubmitFo && d.reportStatus === 'Finalized' ? (
              <Button loading={submitMut.isPending} onClick={() => submitMut.mutate()}>FinanzOnline (Outbox)</Button>
            ) : null}
            {canExport && d.reportStatus === 'Finalized' && !d.supersededByReportId ? (
              <Button onClick={() => correctionMut.mutate()} loading={correctionMut.isPending}>Korrektur (neuer Bericht)</Button>
            ) : null}
            <Button onClick={() => router.push('/reporting/jahresbericht')}>Zur Liste</Button>
          </Space>
        }
      />
      ...
      <Card title="Status" style={{ marginBottom: 16 }}>
        <Descriptions column={1} size="small" bordered>
          <Descriptions.Item label="Umfang"><Tag>{d.scopeKind}</Tag> {d.scopeKind === 'Company' ? 'alle Kassen' : d.registerNumber ?? d.cashRegisterId}</Descriptions.Item>
          <Descriptions.Item label="Bericht"><Tag color={d.reportStatus === 'Finalized' ? 'blue' : 'gold'}>{d.reportStatus}</Tag>{d.correction.isCorrection ? <Tag color="orange">Korrektur</Tag> : null}</Descriptions.Item>
          ...
          {d.correction.supersedesReportId ? (
            <Descriptions.Item label="Vorgänger"><Link href={`/reporting/jahresbericht/${d.correction.supersedesReportId}`}>{d.correction.supersedesReportId}</Link></Descriptions.Item>
          ) : null}
          ...
          {d.submission.externalReferenceId ? <Descriptions.Item label="Referenz">{d.submission.externalReferenceId}</Descriptions.Item> : null}
          {showHashes ? <Descriptions.Item label="Snapshot-Hash">{d.snapshotHash}</Descriptions.Item> : null}
        </Descriptions>
      </Card>

      <Card title="Verknüpfte Monatsberichte (final)" style={{ marginBottom: 16 }}>
        <Table rowKey="monatsberichtId" size="small" pagination={false} dataSource={d.summary.linkedFinalizedMonatsberichte} columns={linkedCols} />
      </Card>

      <Card title="Jahressummen" style={{ marginBottom: 16 }}>
        <Descriptions column={2} size="small" bordered>
          <Descriptions.Item label="Monatsberichte (Anzahl)">{agg.linkedMonthlyReportCount}</Descriptions.Item>
          <Descriptions.Item label="Monate / abgedeckt">{agg.distinctMonthsCovered} / {agg.expectedMonthsInYear}</Descriptions.Item>
          <Descriptions.Item label="Brutto (aus Monatsberichten)">{agg.grossSalesAmount.toFixed(2)}</Descriptions.Item>
      ...
      {(profile === 'accountingReport' || profile === 'legalComplianceExport' || profile === 'diagnosticPackage') ? (
        <Card title="Rohdaten (PaymentDetails, Jahresfenster)" style={{ marginBottom: 16 }}>
          <Descriptions column={2} size="small" bordered>
          ...
          <Descriptions column={1} size="small" bordered style={{ marginTop: 8 }}>
            <Descriptions.Item label="Δ Brutto (Monate − Roh)">{adj.grossDeltaMonthlyVsRaw.toFixed(2)}</Descriptions.Item>
            <Descriptions.Item label="Prüfung">{adj.requiresReview ? <Tag color="warning">Abweichung</Tag> : <Tag>ok</Tag>}{adj.noteDe ? ` — ${adj.noteDe}` : ''}</Descriptions.Item>
          </Descriptions>
        </Card>
      ) : null}
      ...
      <Card title="Hinweise" style={{ marginBottom: 16 }}>
        {d.summary.warnings?.length ? (
          <ul>
            {d.summary.warnings.map((w) => (<li key={w}><Typography.Text type="warning">{w}</Typography.Text></li>))}
          </ul>
        ) : (
      ...
      {showTrace ? (
        <Card title="Trace (Compliance)">
          <Typography.Paragraph type="secondary">
            Annual snapshot finalizasyon sonrası immutable kalır; düzeltmeler yeni rapor satırı olarak zincire eklenir.
