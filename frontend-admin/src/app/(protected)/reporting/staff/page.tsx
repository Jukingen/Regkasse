...
export default function StaffPerformanceReportingPage() {
  ...
  const formatRatio = useCallback(
    (v: number) =>
      formatNumber(Number(v ?? 0), formatLocale, {
  ...
  const formatPct = useCallback(
    (v: number) =>
      formatPercent(Number(v ?? 0), formatLocale, {
  ...
  const dayStaffColumns: ColumnsType<StaffPerformanceLocalDayStaffDto> = [
    ...
    {
      title: t('adminShell.staffPerformance.colCashierId'),
      dataIndex: 'cashierId',
      ellipsis: true,
    },
    { title: t('adminShell.staffPerformance.colSalesCount'), dataIndex: 'saleTransactionCount' },
  ...
  return (
    <Space orientation="vertical" size="large" style={{ width: '100%' }}>
      ...
      <Spin spinning={reportQ.isLoading}>
        ...
        {includePerStaffPerDay ? (
          <Card
            title={t('adminShell.staffPerformance.tableDayStaffTitle')}
            style={{ marginTop: 16 }}
          >
            <Table
