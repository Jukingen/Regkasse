...
import {
  ...
  ReportSelector,
} from '@/features/reports/components/ReportSelector';
...
export default function ComplianceReportingPage() {
  ...
  const reconciliationQ = useQuery({
    ...
    queryFn: () => fetchDailyReconciliation({ businessDate: businessDateParam, cashRegisterId }),
    ...
    },
    {
      ...
      render: (_: unknown, row) => (
        <Space size={4}>
          ...
          {row.hasDuplicates ? (
            <Tag color="error">{t('reporting.compliance.tags.duplicates')}</Tag>
          ) : null}
  ...
  const movementCols: ColumnsType<{
    productId: string;
    productName: string;
    quantitySold: number;
    revenue: number;
        ...
        {tab === 'reconciliation' ? (
          <Col xs={24} md={8}>
            <Typography.Text type="secondary">
              {t('reporting.compliance.businessDate')}
            </Typography.Text>
            <DatePicker
          ...
          <Col xs={24} md={12}>
            <Typography.Text type="secondary">
              {t('reporting.compliance.dateRange')}
            </Typography.Text>
            <RangePicker
      ...
      <Row gutter={16}>
        ...
        <Col xs={24} md={19}>
          ...
          <Tabs
            ...
            activeKey={tab}
            onChange={(k) => setTab(k as ReportCategoryId)}
            items={[
              {
                key: 'reconciliation',
                ...
                children: (
                  <Spin spinning={reconciliationQ.isLoading}>
                    ...
                    <Row gutter={16}>
                      <Col xs={12} md={6}>
                        <Statistic
                          title={t('reporting.compliance.stats.cash')}
                          value={money(rec?.cashTotal ?? 0)}
                        />
                      </Col>
                      <Col xs={12} md={6}>
                        <Statistic
                          title={t('reporting.compliance.stats.card')}
                          value={money(rec?.cardTotal ?? 0)}
                        />
                      </Col>
                      <Col xs={12} md={6}>
                        <Statistic
                      ...
                      <Col xs={12} md={6}>
                        <Statistic
                          ...
                          value={rec?.cashDifference != null ? money(rec.cashDifference) : '—'}
                        />
                      </Col>
                      <Col xs={12} md={6}>
                        <Statistic
                          ...
                          value={
                            rec?.isReconciled
                              ? t('reporting.compliance.yes')
                              : t('reporting.compliance.no')
                          }
                        />
                      </Col>
                    </Row>
                  ...
                  <Spin spinning={tseQ.isLoading}>
                    {tseQ.data?.operatorNoteDe ? (
                      <Alert
                        type="info"
                        showIcon
                        title={tseQ.data.operatorNoteDe}
                        style={{ marginBottom: 12 }}
                      />
                    ) : null}
                  ...
                  <Spin spinning={offlineQ.isLoading}>
                    {offlineQ.data?.operatorNoteDe ? (
                      <Alert
                        type="info"
                        showIcon
                        title={offlineQ.data.operatorNoteDe}
                        style={{ marginBottom: 12 }}
                      />
                    ) : null}
                    <Row gutter={16} style={{ marginBottom: 16 }}>
                    ...
                    <Table
                      ...
                      columns={[
                        {
                          title: t('reporting.compliance.columns.register'),
                    ...
                    <Table
                      ...
                      columns={[
                        ...
                        {
                          title: t('reporting.compliance.columns.error'),
                          dataIndex: 'lastError',
                          ellipsis: true,
                        },
                      ]}
                ...
                children: (
                  <Spin spinning={movementQ.isLoading}>
                    ...
                    <Typography.Title level={5}>
                      {t('reporting.compliance.movement.topRevenue')}
                    </Typography.Title>
                    <Table
                      size="small"
                      rowKey="productId"
                      ...
                      dataSource={
                        movementQ.data?.topSellingByRevenue ?? movementQ.data?.lines ?? []
                      }
                      pagination={{ pageSize: 15 }}
                      style={{ marginBottom: 16 }}
                    />
                    <Typography.Title level={5}>
                      {t('reporting.compliance.movement.slowMovers')}
                    </Typography.Title>
                    <Table
                      size="small"
