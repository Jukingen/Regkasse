...
export default function KassenverwaltungPage() {
  ...
  const { selectedRegisterId, setSelectedRegisterId } = useCashRegisterSelection({
  ...
  const groupedRegisters = useMemo(() => {
    ...
    return Array.from(groups.entries())
      ...
      .sort((a, b) =>
        (a.tenant?.name ?? a.tenantId ?? '').localeCompare(b.tenant?.name ?? b.tenantId ?? '', 'de')
      );
            ...
            <Space orientation="vertical" size="large" style={{ width: '100%' }}>
              {groupedRegisters.map((group) => (
                <div key={group.tenantId ?? 'unknown-tenant'}>
                  <Space wrap align="center" style={{ marginBottom: 12, width: '100%' }}>
