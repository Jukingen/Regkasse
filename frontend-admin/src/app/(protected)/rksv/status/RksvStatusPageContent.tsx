...
import { Alert, Button, Card, Col, Divider, Row, Space, Tag, Tooltip, Typography } from 'antd';
...
export default function RksvStatusPage() {
  ...
  const { parsed: foEnv, devParseDebug: foEnvDevDebug } = useRksvFinanzOnlineEnvironment();
        ...
        <Alert
          ...
          description={
            <Typography.Paragraph style={{ marginBottom: 0 }}>
              Nullbeleg, Startbeleg, Monatsbeleg, Jahresbeleg und Endbeleg (Schlussbeleg) finden Sie
              unter <Link href="/rksv/sonderbelege">RKSV Sonderbelege</Link>.
            </Typography.Paragraph>
          }
        />
      ) : null}
          ...
          <Col xs={24} md={12}>
            <Card title={OPERATOR_RKSV_GENERAL_STATUS_COPY.tseCardTitle} size="small">
              ...
              {tseStatus?.certificateStatus && (
                <p>
                  {OPERATOR_RKSV_GENERAL_STATUS_COPY.tseCertificateLabel}:{' '}
                  {tseStatus.certificateStatus}
                </p>
              )}
            ...
            <Card title={OPERATOR_RKSV_GENERAL_STATUS_COPY.foCardTitle} size="small">
              ...
              >
                <Tag
                  ...
                  icon={
                    foStatus?.finanzOnlineTransportsSimulated ? undefined : foStatus?.isAuthoritative ===
                      false ? undefined : foStatus?.isConnected ? (
                      <CheckCircleOutlined />
                    ) : (
                      <CloseCircleOutlined />
                    )
              ...
              {foStatus?.pendingInvoices !== undefined && (
                <p style={{ marginTop: 8 }}>
                  {OPERATOR_RKSV_GENERAL_STATUS_COPY.foPendingInvoicesLabel}:{' '}
                  {foStatus.pendingInvoices}
                </p>
              )}
              {foStatus?.lastSync && (
                <p>
