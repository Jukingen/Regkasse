...
import {
  useGetApiAuditLog,
  useGetApiAuditLogCorrelationCorrelationId,
} from '@/api/generated/audit-log/audit-log';
...
export default function RksvVerificationsPage() {
  ...
  const { data: correlationData, isLoading: correlationLoading } =
    useGetApiAuditLogCorrelationCorrelationId(correlationId ?? '', {
  ...
  const tablePagination = useCorrelation
    ? {
        ...
        showTotal: (_total: number, range: [number, number]) =>
          OPERATOR_VERIFICATIONS_COPY.verificationsCorrelationPaginationShowTotal(
            range,
  ...
  const columns = useMemo(
    () => [
        ...
        render: (_: unknown, r: AuditLogEntryDto) => {
          ...
          return (
            <Space orientation="vertical" size={2}>
              ...
              <Link href={buildVerificationsAuditHref(c)}>
                {OPERATOR_VERIFICATIONS_COPY.filterByThisCorrelationLabel}
              </Link>
            </Space>
          );
        },
      ...
      },
      {
        ...
        width: 168,
        render: (ts: string) =>
          ts ? formatAuditTimestamp(ts, formatLocale) : FORMAT_EMPTY_DISPLAY,
      },
      {
        ...
        width: 120,
        render: (_: unknown, r: AuditLogEntryDto) =>
          r.actorDisplayName ?? r.userId ?? FORMAT_EMPTY_DISPLAY,
      },
      {
    ...
    return (
      <div style={{ padding: '8px 12px 16px', background: '#fafafa' }}>
        ...
        <Typography.Paragraph style={{ marginBottom: 8, fontSize: 12 }}>
          <strong>{tv('expand.treffergrundHeading')}</strong>{' '}
          {describeVerificationsKeywordMatchDe(r)}
        ...
        <Descriptions bordered size="small" column={1}>
          <Descriptions.Item label={tv('expand.fieldAction')}>
            {r.action ?? FORMAT_EMPTY_DISPLAY}
          </Descriptions.Item>
          <Descriptions.Item label={tv('expand.fieldEntityType')}>
            {r.entityType ?? FORMAT_EMPTY_DISPLAY}
          </Descriptions.Item>
          ...
          <Descriptions.Item label={tv('expand.fieldEndpoint')}>
            {r.endpoint ?? FORMAT_EMPTY_DISPLAY}
          </Descriptions.Item>
          <Descriptions.Item label={tv('expand.fieldHttpMethod')}>
            {r.httpMethod ?? FORMAT_EMPTY_DISPLAY}
          </Descriptions.Item>
                ...
                ? [
                    {
                      ...
                      children: (
                        <Typography.Paragraph
                          copyable={r.requestData ? { text: r.requestData } : false}
                          style={{
                            marginBottom: 0,
                            fontSize: 11,
                ...
                ? [
                    {
                      ...
                      children: (
                        <Typography.Paragraph
                          copyable={r.responseData ? { text: r.responseData } : false}
                          style={{
                            marginBottom: 0,
                            fontSize: 11,
                            fontFamily: 'monospace',
        ...
        <Space orientation="vertical" size={6}>
          ...
          <Space wrap size={[8, 8]}>
            ...
            {ctx ? (
              <Link href={buildVerificationsAuditHref(ctx)}>
                {OPERATOR_VERIFICATIONS_COPY.filterByThisCorrelationLabel}
              </Link>
            ) : null}
          </Space>
