...
export default function OfflineOrdersPage() {
  ...
  const { data, isLoading, refetch, isFetching, isError, error } = useGetApiAdminOfflineOrders(
    listParams,
    {
  ...
  const columns: ColumnsType<AdminOfflineOrderRowDto> = [
    ...
    },
    {
      ...
      render: (status: string) => {
        ...
        return <Tag color={statusColor(status)}>{known ? tp(labelKey) : status}</Tag>;
      },
    },
    {
      title: tp('colAction'),
  ...
  return (
    <Space orientation="vertical" size="large" style={{ width: '100%', paddingBottom: 24 }}>
      ...
      <Alert
        type="warning"
        showIcon
        title={tp('warningTitle')}
        description={tp('warningDescription')}
      />
