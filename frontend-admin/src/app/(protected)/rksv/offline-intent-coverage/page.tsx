...
export default function OfflineIntentCoveragePage() {
  const { t } = useI18n();
  const [dateRange, setDateRange] = useState<[Dayjs, Dayjs]>([dayjs().subtract(1, 'day'), dayjs()]);
  ...
  const byRegisterColumns = [
    ...
    },
    {
      ...
      key: 'riskScore',
      width: 120,
      render: (v: number) => <Tag color={riskTagColor(v)}>{v.toFixed(4)}</Tag>,
    },
  ];
        ...
        <>
          {coverage?.lowCoverageAlert ? (
            <Alert
              ...
              description={
                coverage?.alertReason ?? t('rksvHub.offlineIntentCoveragePage.thresholdFallback')
              }
            />
          ) : null}
            ...
            <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
              <Space wrap align="center">
                <Typography.Text strong>
                  {t('rksvHub.offlineIntentCoveragePage.filterDateRangeUtc')}
                </Typography.Text>
                <RangePicker
              ...
              <Space wrap align="center">
                <Typography.Text strong>
                  {t('rksvHub.offlineIntentCoveragePage.filterRegisterOptional')}
                </Typography.Text>
                <Select
                  allowClear
              ...
              <Space wrap align="center">
                <Typography.Text strong>
                  {t('rksvHub.offlineIntentCoveragePage.topRiskLimit')}
                </Typography.Text>
                <InputNumber
                  min={1}
                  max={100}
                ...
                <Typography.Text type="secondary">
                  {t('rksvHub.offlineIntentCoveragePage.limitHint')}
                </Typography.Text>
              </Space>
            </Space>
          </Card>
          ...
          <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
            <Col xs={24} md={6}>
              <Card size="small">
                <Statistic
                  title={t('rksvHub.offlineIntentCoveragePage.statSamples')}
            ...
            <Col xs={24} md={6}>
              <Card size="small">
                <Statistic
                  ...
                  value={`${formatPercent01AsPct(coverage?.deviceIdMissingRate ?? 0)} / ${formatPercent01AsPct(
