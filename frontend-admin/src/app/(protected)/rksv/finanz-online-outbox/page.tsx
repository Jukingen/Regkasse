...
export default function FinanzOnlineOutboxPage() {
  ...
  const modeTag = useCallback((modeVal: string | null | undefined) => {
  ...
  const bucketOptions = useMemo(
    () => [
      ...
      { value: 'ProtocolFailure', label: t('finanzOnlineOutbox.buckets.protocolFailure') },
      {
        ...
        label: t('finanzOnlineOutbox.buckets.manualReviewRequired'),
      },
  ...
  const columns: ColumnsType<FinanzOnlineOutboxItemDto> = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (_: unknown, r) => {
          const hintKey =
            r.status === 'ProtocolSuccess'
              ? protocolSuccessStatusHintKey(r.transportPathKind)
              : null;
          return (
            <Space orientation="vertical" size={0}>
              ...
              {hintKey ? (
                <Typography.Text
                  type={
                    isSimulatedFinanzOnlineTransportPath(r.transportPathKind)
                      ? 'warning'
                      : 'secondary'
                  }
                  style={{ fontSize: 11 }}
          ...
          <Button
            ...
            onClick={() => {
              void queryClient.invalidateQueries({
                queryKey: rksvAdminQueryKeys.finanzOnlineOutbox.base,
              });
              void queryClient.invalidateQueries({
                queryKey: rksvAdminQueryKeys.finanzOnlineOutbox.readiness(),
              });
      ...
      >
        <Typography.Paragraph style={{ marginBottom: 8, maxWidth: 960 }}>
          ...
          <Link href="/rksv/finanz-online-queue">
            {t('finanzOnlineOutbox.page.leadLegacyQueueLink')}
          </Link>
          .
        </Typography.Paragraph>
        <Typography.Paragraph
          type="secondary"
          style={{ marginBottom: 0, maxWidth: 960, fontSize: 12 }}
        >
          ...
          <>
            ...
            <Descriptions bordered size="small" column={{ xs: 1, sm: 2 }}>
              ...
              <Descriptions.Item label={t('finanzOnlineOutbox.readiness.scenarioDelay')}>
                {readiness.simulationFixedDelayMs != null
                  ? String(readiness.simulationFixedDelayMs)
                  : emDash}
              </Descriptions.Item>
            ...
            {readiness.summary?.trim() ? (
              <Typography.Paragraph style={{ marginTop: 12, marginBottom: 0 }}>
                <Typography.Text strong>
                  {t('finanzOnlineOutbox.readiness.apiSummary')}:{' '}
                </Typography.Text>
      ...
      {isLoading && items.length === 0 ? (
        ...
        <Table<FinanzOnlineOutboxItemDto>
          ...
          pagination={{
            ...
            showTotal: () =>
              t('finanzOnlineOutbox.pagination.showTotal', { total, shown: items.length }),
          }}
          onRow={(record) => ({
          ...
          <>
            <Descriptions bordered size="small" column={1}>
              ...
              <Descriptions.Item label={t('finanzOnlineOutbox.drawer.fields.statusDisplay')}>
                <Tag
                  color={finanzOnlineOutboxStatusTagColor(
                    displayRow.status,
                    displayRow.transportPathKind
                  )}
                >
