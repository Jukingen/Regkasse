...
export default function RksvCmcCertificatePage() {
  ...
  const tc = useCallback(
    (path: string, options?: Record<string, string | number>) =>
      t(`rksvHub.cmcCertificate.${path}`, options),
    [t]
  );
        ...
        description={
          <Space orientation="vertical" size={8} style={{ width: '100%' }}>
            ...
            <Typography.Paragraph type="secondary" style={{ marginBottom: 0, fontSize: 12 }}>
              ...
              <Link href="/rksv/finanz-online-queue">{tc('scopeBannerP2Link')}</Link>{' '}
              {tc('scopeBannerP2After')}
            </Typography.Paragraph>
          </Space>
        }
      />
      ...
      <Card size="small" title={tc('cardSnapshotTitle')}>
        <Space orientation="vertical" size={12} style={{ width: '100%' }}>
          ...
          <Descriptions
            ...
            size="small"
          >
            <Descriptions.Item label={tc('descSerial')}>
              {tseStatus?.serialNumber ?? FORMAT_EMPTY_DISPLAY}
            </Descriptions.Item>
            <Descriptions.Item label={tc('descKassenId')}>
              {tseStatus?.kassenId ?? FORMAT_EMPTY_DISPLAY}
            </Descriptions.Item>
          </Descriptions>
      ...
      <Card size="small" title={tc('cardPlannedTitle')}>
        <Space orientation="vertical" size={8} style={{ width: '100%' }}>
          <Alert
            type="info"
            showIcon
            title={tc('plannedValidityTitle')}
            description={tc('plannedValidityDesc')}
          />
          <Alert
            type="info"
            showIcon
            title={tc('plannedChainTitle')}
            description={tc('plannedChainDesc')}
          />
        </Space>
      </Card>
    </>
