...
export function LicenseReportsCard({ enabledLicenseFeatures }: Props) {
  const { message } = useAntdApp();
    ...
    const canExport =
        deploymentLicenseAllows(enabledLicenseFeatures, LICENSE_DEPLOYMENT_FEATURE.AdminLicenseManage) &&
        ...
        <Space orientation="vertical" size={16} style={{ width: '100%' }}>
            ...
            <Card title={t('license.reports.filtersTitle')}>
                ...
                >
                    ...
                    <Form.Item name="maskLicenseKeys" label={t('license.reports.maskLicenseKeys')} valuePropName="checked">
                        <Switch />
                    </Form.Item>
                    <Form.Item>
                        <Space wrap>
            ...
            <Card title={t('license.reports.summaryTitle')}>
                {summaryQuery.isLoading ? (
                    ...
                    <Descriptions bordered column={1} size="small">
                        ...
                        <Descriptions.Item label={t('license.reports.issuedRevoked')}>{s.issuedRevoked}</Descriptions.Item>
                        <Descriptions.Item label={t('license.reports.issuedCancelled')}>{s.issuedCancelled}</Descriptions.Item>
                        <Descriptions.Item label={t('license.reports.issuedDeleted')}>{s.issuedDeleted}</Descriptions.Item>
                        <Descriptions.Item label={t('license.reports.expiring30')}>{s.expiringWithin30Days}</Descriptions.Item>
                        <Descriptions.Item label={t('license.reports.expiring15')}>{s.expiringWithin15Days}</Descriptions.Item>
                        <Descriptions.Item label={t('license.reports.expiring7')}>{s.expiringWithin7Days}</Descriptions.Item>
