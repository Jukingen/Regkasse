    ...
    () => [
      ...
      },
      {
        ...
        render: (_, row) => {
          ...
          if (row.isRevoked)
            return <Tag color="red">{t('license.issued.columns.statusRevoked')}</Tag>;
    ...
    <Card title={t('license.issued.title')}>
      ...
      >
        <Typography.Paragraph type="secondary">
          {t('license.issued.super.extendResultHelp')}
        </Typography.Paragraph>
        {extendResult?.licenseKey ? (
          <Descriptions bordered column={1} size="small" style={{ marginTop: 12 }}>
            <Descriptions.Item label={t('license.generation.result.licenseKey')}>
              <Typography.Paragraph
                copyable={{ text: extendResult.licenseKey }}
            ...
            <Descriptions.Item label={t('license.generation.result.expiry')}>
              {extendResult.expiryAtUtc ? formatGermanDateTime(extendResult.expiryAtUtc) : '—'}
            </Descriptions.Item>
      ...
      <IssuedLicenseUpgradeModal row={upgradeModalRow} onClose={() => setUpgradeModalRow(null)} />
    </Card>
  );
}
...
export default function AdminLicensePage() {
  ...
  const { showAllTenantLicensesSection, showTenantLicenseSection, showDeploymentSection } =
    resolveLicensePageSectionVisibility(user, {
  ...
  const { refresh: refreshLicensePage, isFetching: isLicensePageRefreshing } =
    useLicensePageRefresh({
  ...
  const licensePagePrefill = useMemo(
    () => readAdminLicensePagePrefill(searchParams),
    [searchParams]
  );
              ...
              <Row gutter={[16, 16]}>
                <Col xs={24} lg={14}>
                  ...
                  <Descriptions bordered column={1} size="small">
                    ...
                    <Descriptions.Item label={t('license.simpleUi.validUntil')}>
                      {s?.expiryDate ? formatGermanDateTime(s.expiryDate) : '—'}
                    </Descriptions.Item>
                    <Descriptions.Item label={t('license.simpleUi.daysRemaining')}>
