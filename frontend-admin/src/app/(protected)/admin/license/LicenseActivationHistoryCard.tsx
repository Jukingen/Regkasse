...
export function LicenseActivationHistoryCard() {
  ...
  const onApply = (values: FilterFormValues) => {
    const next: LicenseActivationAttemptsListParams = {
      ...
      machineFingerprint: values.machineFingerprint?.trim()
        ? values.machineFingerprint.trim()
        : undefined,
    };
  ...
  return (
    ...
    >
      <Typography.Paragraph type="secondary">
        {t('license.activationHistory.subtitle')}
      ...
      <Form form={form} layout="vertical" onFinish={onApply} style={{ marginBottom: 16 }}>
        <Space wrap align="start">
          <Form.Item
            name="licenseKey"
            label={t('license.activationHistory.licenseKeyFilter')}
            style={{ minWidth: 260 }}
          >
            <Input
              allowClear
              placeholder={t('license.activationHistory.licenseKeyPlaceholder')}
              autoComplete="off"
            />
          </Form.Item>
          <Form.Item
          ...
          <Form.Item
            name="status"
            label={t('license.activationHistory.statusFilter')}
            initialValue="all"
          >
            <Select
              style={{ width: 160 }}
          ...
          <Form.Item
            name="onlyFailed"
            label={t('license.activationHistory.onlyFailedToggle')}
