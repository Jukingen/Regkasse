...
import { Alert, Button, DatePicker, Descriptions, Input, Modal, Space, Typography } from 'antd';
    ...
    onError: (err: unknown) => {
      if (axios.isAxiosError(err)) {
        ...
        const data = err.response?.data as
          { message?: string } | GenerateLicenseResponse | undefined;
        const msg =
        ...
        {row ? (
          <>
            <Typography.Paragraph type="secondary">
              {t('license.issued.upgrade.customerHint', { customer: row.customerName })}
          ...
          <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
            ...
            <Descriptions bordered column={1} size="small">
              ...
              <Descriptions.Item label={t('license.generation.result.expiry')}>
                {outcome.expiryAtUtc ? formatGermanDateTime(outcome.expiryAtUtc) : '—'}
              </Descriptions.Item>
            </Descriptions>
          </Space>
        ) : null}
