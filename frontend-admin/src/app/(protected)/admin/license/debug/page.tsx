...
import { getMandantLicenseHistory, licenseHistoryQueryKeys } from '@/features/license/api/licenseHistory';
...
export default function LicenseDebugPage() {
    ...
    const comparisonRows = useMemo(() => {
        ...
        return [
            ...
            },
            {
                ...
                daysRemaining: detail
                    ? resolveTenantLicenseStatus(detail).daysRemaining
                    : '—',
    ...
    const refreshAll = async () => {
        await Promise.all([
            queryClient.invalidateQueries({ queryKey: tenantLicenseUnifiedQueryKeyFor(effectiveTenantId) }),
            queryClient.invalidateQueries({ queryKey: tenantLicenseQueryKeys.detail(effectiveTenantId ?? '') }),
            queryClient.invalidateQueries({ queryKey: licenseHistoryQueryKeys.detail(effectiveTenantId ?? '') }),
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {isLoading && !unifiedQuery.data ? (
                ...
                <>
                    <Card title={t('license.debug.comparisonTitle')}>
                        <Table
                            ...
                            columns={[
                                ...
                                { title: t('license.debug.columns.endpoint'), dataIndex: 'endpoint', key: 'endpoint', ellipsis: true },
                                { title: t('license.debug.columns.kind'), dataIndex: 'kind', key: 'kind', render: (v) => <Tag>{String(v)}</Tag> },
                                { title: t('license.debug.columns.daysRemaining'), dataIndex: 'daysRemaining', key: 'daysRemaining' },
                                { title: t('license.debug.columns.validUntil'), dataIndex: 'validUntil', key: 'validUntil', ellipsis: true },
                                { title: t('license.debug.columns.canAccess'), dataIndex: 'canAccess', key: 'canAccess' },
                            ]}
                        />
                    </Card>
                    ...
                    {effectiveTenantId ? (
                        <LicenseHistory tenantId={effectiveTenantId} />
                    ) : null}
                </>
            )}
        </div>
