...
import { FiskalyOperationHistory } from '@/features/fiskaly/FiskalyOperationHistory';
...
export default function FiskalyHistoryPage() {
  const { t } = useI18n();
  ...
  const breadcrumbs = canConfigure
    ? buildPlatformAdminBreadcrumbs(t, 'securityTse', [
        ...
        { title: t('tseFiskaly.history.pageTitle') },
      ])
    : [
  ...
  return (
    <div>
      ...
      />
      <Typography.Paragraph type="secondary">{t('tseFiskaly.history.pageSubtitle')}</Typography.Paragraph>
      <Card size="small" style={{ marginBottom: 16 }}>
        <Space>
          {canConfigure ? (
            <Link href="/admin/tse/fiskaly/test">{t('tseFiskaly.test.openPage')}</Link>
          ) : null}
          <Link href="/admin/fiskaly/statistics">{t('tseFiskaly.statistics.openPage')}</Link>
          <Link href="/admin/fiskaly/errors">{t('tseFiskaly.errors.openPage')}</Link>
        </Space>
      </Card>
      <FiskalyOperationHistory />
    </div>
  );
}
