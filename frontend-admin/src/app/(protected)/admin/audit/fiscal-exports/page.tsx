  ...
  const { message } = useAntdApp();
    ...
    const handleServerCsvExport = useCallback(async () => {
        ...
        try {
            ...
            const truncated = String(res.headers?.['x-fiscal-audit-export-truncated'] ?? '').toLowerCase() === 'true';
            const blob = res.data;
            ...
            if (!contentType.includes('text/csv')) {
                try {
                    ...
                    message.error(typeof parsed?.message === 'string' ? parsed.message : t('fiscalExportAudit.export.failed'));
                    return;
                } catch {
        ...
        () => [
            ...
            },
            {
                ...
                render: (_: unknown, r: FiscalExportAuditLogListItem) => {
                    const from = r.exportPeriodFromUtc ? dayjs(r.exportPeriodFromUtc).format('DD.MM.YYYY') : '—';
    ...
    const scopeSummary = useMemo(() => {
        ...
        parts.push(`${t('fiscalExportAudit.filters.exportType')}: ${t(`fiscalExportAudit.exportType.${exportType}`)}`);
        if (downloadRange?.[0] && downloadRange[1]) {
            parts.push(`${downloadRange[0].format('DD.MM.YYYY')}–${downloadRange[1].format('DD.MM.YYYY')}`);
        } else {
        ...
        return parts.join(' · ');
    }, [data?.totalCount, debouncedUserSearch, downloadRange, exportType, formatLocale, page, pageSize, t]);
            ...
            <AdminPageHeader
                ...
                actions={
                    <Space wrap>
                        ...
                        <Tooltip title={canServerExport ? undefined : t('fiscalExportAudit.actions.exportCsvNoPermission')}>
                            <Button icon={<DownloadOutlined />} onClick={handleServerCsvExport} disabled={!canServerExport}>
            ...
            >
                {detailLoading ? (
                    ...
                    <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
                        ...
                        <div>
                            <Typography.Text type="secondary">{t('fiscalExportAudit.detail.role')}</Typography.Text>
                            <div>{detail.userRole}</div>
                        </div>
                        <div>
                            <Typography.Text type="secondary">{t('fiscalExportAudit.detail.requestJson')}</Typography.Text>
                            <pre
                                style={{
                                    maxHeight: 320,
                                    overflow: 'auto',
                        ...
                        <div>
                            <Typography.Text type="secondary">{t('fiscalExportAudit.detail.responseJson')}</Typography.Text>
                            <pre
                                style={{
                                    maxHeight: 240,
                                    overflow: 'auto',
                        ...
                        {detail.errorDetails ? (
                            <div>
                                <Typography.Text type="secondary">{t('fiscalExportAudit.detail.errors')}</Typography.Text>
