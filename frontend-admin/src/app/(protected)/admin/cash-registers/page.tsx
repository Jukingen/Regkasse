...
export default function AdminCashRegistersPage() {
  ...
  const allowHardDeleteUi = canHardDelete && (capabilitiesQuery.data?.allowHardDelete ?? false);
    ...
    (nextReason?: string) => {
      ...
      decommissionMutation.mutate({
        ...
        reason:
          ...
          'Admin tenant cash register decommission',
      });
    },
  ...
  const bulkDecommissionMutation = useMutation({
    ...
    onSuccess: async ({ success, failed }) => {
      if (failed === 0) {
        ...
        message.warning(t('cashRegisters.bulk.partialSuccess', { success, failed }));
      }
        ...
        <Card title={t('cashRegisters.pageTitle')}>
          <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
            ...
            <Link href="/kassenverwaltung">
              {t('cashRegisters.adminPage.openKassenverwaltung')}
            </Link>
          </Space>
        </Card>
      </AdminPageShell>
  ...
  return (
    <AdminPageShell>
      ...
      <BulkDecommissionModal
        ...
        onConfirm={(reason) => bulkDecommissionMutation.mutate({ registers: selectedRows, reason })}
