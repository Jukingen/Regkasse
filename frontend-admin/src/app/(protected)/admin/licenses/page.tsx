...
export default function AdminTenantLicensesPage() {
  ...
  const canAccess = isSuperAdmin(user?.role) || hasPermission(user, PERMISSIONS.LICENSE_VIEW);
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (licenseStatusFilter !== 'all' && row.resolvedLicenseStatus.kind !== licenseStatusFilter) {
        return false;
      }
  ...
  const summary = useMemo(
    () => ({
      ...
      graceReadOnly: rows.filter((row) => row.resolvedLicenseStatus.kind === 'grace_readonly')
        .length,
  ...
  const columns = useMemo<ColumnsType<TenantLicenseTableRow>>(
    () => [
      ...
      },
      {
        ...
        width: 200,
        sorter: (a, b) =>
          a.resolvedLicenseStatus.daysRemaining - b.resolvedLicenseStatus.daysRemaining,
        render: (_: number | null | undefined, row) => {
          ...
          if (status.daysRemaining <= 0) {
            return <span style={{ color: '#ff4d4f' }}>{t('license.phase.labels.expired')}</span>;
          }

          const { color, progressColor } = getDaysColumnTone(status.daysRemaining);
          const percent = Math.min((status.daysRemaining / LICENSE_PROGRESS_TOTAL_DAYS) * 100, 100);
        ...
        render: (_, row) => (
          <Space>
            ...
            <Tooltip title={t('tenants.actions.manageLicense')}>
              <Button icon={<EditOutlined />} size="small" onClick={() => setExtendTenant(row)} />
            </Tooltip>
    ...
    [formatLocale, sendReminderEmail, t, reminderMutation.isPending, reminderTenantId]
  );
    ...
    <AdminPageShell>
      ...
      <Card>
        <Table<TenantLicenseTableRow>
          ...
          locale={{
            emptyText: (
              <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t('tenants.page.empty')} />
            ),
          }}
