...
function badgeTag(statusBadge: string, t: (k: string) => string): React.ReactNode {
  ...
  if (s === 'critical') return <Tag color="red">{t('timeSync.badge.critical')}</Tag>;
  if (s === 'warning') return <Tag color="orange">{t('timeSync.badge.warning')}</Tag>;
  return <Tag color="green">{t('timeSync.badge.synchronized')}</Tag>;
}
