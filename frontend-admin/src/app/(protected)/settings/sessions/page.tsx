...
type PendingConfirm =
    | { kind: 'revoke'; session: ActiveSession }
    | { kind: 'revokeOthers' }
    | null;
...
export default function ActiveSessionsPage() {
    ...
    const relativeLocale =
        textLocale === 'tr' ? 'tr' : textLocale === 'en' ? 'en' : 'de';
    ...
    const handleConfirm = async () => {
        ...
        try {
            if (pendingConfirm.kind === 'revoke') {
                ...
                message.success(
                    t('common.auth.sessions.terminatedOthers', { count: String(count) }),
                );
            }
    ...
    const columns: ColumnsType<ActiveSession> = [
        {
            ...
            key: 'deviceName',
            render: (_, record) =>
                record.deviceName?.trim() || t('common.auth.sessions.unknownDevice'),
        },
        {
        ...
        },
        {
            ...
            render: (value: string) =>
                value ? dayjs(value).locale(relativeLocale).fromNow() : '—',
        },
        {
            ...
            render: (_, record) => (
                <Space wrap>
                    {record.isCurrent ? (
                        <StatusBadge
                            status="info"
                            label={t('common.auth.sessions.thisDevice')}
                        />
                    ) : null}
                    <StatusBadge
