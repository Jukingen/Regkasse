...
export type ImpersonationHandoffParseResult =
    ...
    | { ok: false; reason: 'missing_token' | 'missing_tenant' | 'tenant_mismatch' | 'invalid_token' | 'not_impersonation' | 'expired' };
...
export function buildTenantSubdomainOrigin(tenantSlug: string, protocol: 'https' | 'http' = 'https'): string {
...
export function parseImpersonationHandoffFromHash(
    ...
    const tenantId =
        typeof tenantIdClaim === 'string' && tenantIdClaim.trim().length > 0 ? tenantIdClaim.trim() : null;
